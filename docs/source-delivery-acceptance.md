...
## 2026-09-21 后续决定：收窄稳定来源

用户决定将 VentureBeat、机器之心、36kr、Stack Overflow 退出日常采集，不再作为待修复的交付阻塞项。已停用本机对应 13 个入口（3 个媒体入口、10 个 Stack Overflow 标签/排序入口），取消其待执行任务；已有 6,120 条资料及来源关系保持不变。默认来源清单同步设为不启用，新批次排除这些入口，目录重新初始化后仍保留停用选择。历史批次的错误记录不改写。本轮未额外采集、未新增来源、未续用上轮付费或模型授权。

后续以“来源稳定、正文可读、AI 相关性有依据、原始出处可追溯”为基础供给标准；不以信源数或抓取数作为质量验收。剩余来源仍须依据实际运行和内容质量判断，不因退出故障来源就宣称全部内容合格。
