...
export function harnessChildEnv(env, runDir, kind) {
  ...
  return {
    ...
    XENHO_EXPERT_PERSONA: "你是 Xenho OS 的专业内容顾问。用户是主创。只读研究、保留来源、提交结构化报告；不得修改正文、不得编造经历或证据。",
    XENHO_CONTEXT_FILE: path.join(runDir, "context.json"),
    XENHO_REPORT_FILE: path.join(runDir, "report.json"),
    XENHO_SESSION_ROOT: path.join(runDir, "sessions"),
...
export async function createHarnessRun({ env, runDir, kind, prompt, onNotification, onHarness }) {
  ...
  if (!info.configured) {
    throw Object.assign(new Error("专家执行模型尚未配置"), {
      hint: "到 设置 → 专家执行 填写模型地址、模型名和密钥；普通写作不受影响。",
    });
  }
  ...
  const result = await harness.run(prompt, {
    sessionId: `session-${path.basename(runDir).replace(/[^a-z0-9-]/gi, "")}`,
    onNotification,
  });
