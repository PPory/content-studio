...
export async function* collectCommunity({ channel, checkpoint = {}, request, signal, now = new Date(), budget = 20, mode = 'sync' }) {
 const c = config(channel);
 if (c.adapter === 'github_search_api' || c.platform === 'github') { yield* collectGitHub({ c, checkpoint, request, signal, now, budget, mode }); return; }
...
async function* collectGitHub({ c, checkpoint, request, signal, now, budget, mode }) {
 const end = checkpoint.end || now.toISOString();
 let windows = checkpoint.windows || [{ start: new Date(now.getTime() - (c.initialLookbackDays || 7) * 86400000).toISOString(), end, page: 1 }];
 let requests = 0;
