...
export async function backupBeforeAcquisitionMigration(paths) {
  ...
  try {
    const version=source.pragma('user_version',{simple:true});if(!version||version>=35)return;
    const folder=path.join(paths.backupsDir,'Migration-Points');await fs.mkdir(folder,{recursive:true});
    const file=path.join(folder,`before-intelligence-v35-${Date.now()}-${crypto.randomBytes(4).toString('hex')}.sqlite`);
    ...
    const bytes=await fs.readFile(file);await fs.writeFile(file+'.json',JSON.stringify({fromVersion:version,toVersion:35,sha256:crypto.createHash('sha256').update(bytes).digest('hex'),integrity:'ok',verifiedAt:new Date().toISOString()},null,2));
