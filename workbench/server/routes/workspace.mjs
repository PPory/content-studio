...
import { restoreContent, trashContent } from "../domain/research.mjs";
...
export const workspaceRoutes = [
  ...
  { method: "POST", path: "/api/workspace/projects/:id/trash", handler: guarded(async ({ workspace, res, params }) => { json(res, { ok: true, ...trashContent(workspace, params.id) }); }) },
  { method: "POST", path: "/api/workspace/projects/:id/restore", handler: guarded(async ({ workspace, res, params }) => { restoreContent(workspace, params.id); const project = projectDto(workspace, params.id); if (!project) throw new Error("稿子不存在"); json(res, { ok: true, project }); }) },
