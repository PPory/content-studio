...
export const contentProjectRoutes = [
  ...
  { method: "POST", path: "/api/workspace/projects/:id/plan/choose-angle", handler: guard(async ({ env, workspace, req, res, params }) => { const body = await readJsonBody(req); chooseAngle(workspace, params.id, { angleId: body.angleId, own: body.own, from: body.from === "prev" ? "prev" : "" }); json(res, { ok: true, ...planView(workspace, env, params.id) }); }) },
  ...
  { method: "POST", path: "/api/workspace/projects/:id/plan/choose-structure", handler: guard(async ({ env, workspace, req, res, params }) => { const body = await readJsonBody(req); chooseStructure(workspace, params.id, { structure: Number(body.structure) || 0, title: Number(body.title) || 0, from: body.from === "prev" ? "prev" : "" }); json(res, { ok: true, ...planView(workspace, env, params.id) }); }) },
