...
import { chatSystem, DEFAULT_PROMPTS } from "../lib/prompts.mjs";
...
export const localAiRoutes = [
  { method: "POST", path: "/api/agent/chat", handler: guard(async ({ env, workspace, req, res }) => { const body=await readJsonBody(req); if(!clean(body.message,8_000)) throw Object.assign(new Error("message 不能为空"),{status:400}); const workflow=body.workflow==="interview"?"interview":body.workflow==="brainstorm"?"brainstorm":"general"; const scopeId=scope(`guided-${workflow}`,body.docPath||body.draftTitle||body.docTitle||"global"); const role=chatSystem(workspace.repository.getSetting("prompts",DEFAULT_PROMPTS)); await plainTextAssistant({env,res,input:{scopeId,conversationId:body.sessionId,message:`${role}\n\n${guidedMessage(body)}`,document:{id:clean(body.documentId,180),title:clean(body.draftTitle||body.docTitle,300),body:clean(body.pageContext,20_000),platform:clean(body.platform,80),audience:clean(body.audience,300),selection:body.selection?{text:clean(body.selection,8_000)}:null},materials:[]}}); }) },
  { method: "POST", path: "/api/workspace/ai/writing-assist", handler: guard(async ({ env, req, res }) => { const body=await readJsonBody(req); json(res,{ok:true,...await localWriting(env,body)}); }) },
  { method: "POST", path: "/api/workspace/ai/text-revision", handler: guard(async ({ env, req, res }) => { const body=await readJsonBody(req); if(!clean(body.selected)) throw new Error("请先选择需要修订的文字"); json(res,{ok:true,...await localWriting(env,body)}); }) },
