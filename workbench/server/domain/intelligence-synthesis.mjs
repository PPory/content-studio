...
export async function organizeIntelligenceSources(env,{sources,directions,previous},deps={}){
 ...
 for(const item of response.data.groups.slice(0,10)){
  ...
  const documentCount=intelligenceDocumentCount(sourceIds.map(id=>byId.get(id)));
  if(!documentCount)continue;
  groups.push({key:item.key.slice(0,500),focus:item.focus.slice(0,500),connection:item.connection.slice(0,1500),relationship:documentCount>1?item.relationship:'standalone',sourceIds});keys.add(item.key);sourceIds.forEach(id=>used.add(id));
 }
