...
import { processReview, processingFor } from '../acquisition/review.mjs';
...
export function unifiedSummary(w) {
 ...
 return {pending:(counts.pending||0)+(counts.retry||0),failures:counts.failed||0,permissionRequired:counts.permission_required||0,filtered:counts.filtered||0,processed:counts.done||0};
}
...
export async function executeUnifiedBriefs(w,env,runId,deps={}) {
 ...
 // The local worker is serial; a processing row here belongs to an interrupted prior turn.
 w.db.prepare("UPDATE intel_unified_sources SET status=CASE WHEN attempts>=2 THEN 'failed' ELSE 'retry' END,last_error='上次整理中断，按原次数继续重试' WHERE status='processing'").run();
 const pending=w.db.prepare("SELECT s.* FROM intel_unified_sources u JOIN intel_sources s ON s.id=u.source_id WHERE u.status IN ('pending','retry') AND u.attempts<2 AND s.deleted_at IS NULL ORDER BY u.attempts,u.updated_at,s.created_at,s.id LIMIT 8").all().map(sourceFromRow);
 if(!pending.length){const prior=stepState(w,runId,'unified'),acquisition=stepState(w,runId,'acquire');const acquisitionFailures=acquisition.batchId?w.db.prepare("SELECT count(*) n FROM acquisition_runs WHERE batch_id=? AND status IN ('failed','blocked')").get(acquisition.batchId).n:0;const summary={...prior,...unifiedSummary(w),acquisitionFailures};updateRun(w,runId,summary.failures||acquisitionFailures?'partial':'done',summary.failures||acquisitionFailures?'部分来源或内容待复核，已有推荐保留':'情报已更新；没有新增也保留已有推荐');saveStep(w,runId,'unified','done',summary);return intelligenceRun(w,runId);}
 const sourceIds=pending.map(s=>s.id);
 updateRun(w,runId,'running','筛选相关内容、整理中文导读');
 await processReview(w,env,{confirmed:true,semantic:true,sourceIds,skipReplay:true,skipGrouping:true},deps);
 deps.assertCurrent?.();
 const eligible=[];
 for(const s of pending){const p=processingFor(w,s);let status=!sourcePermission(s,'ai')?'permission_required':!p.readable||p.relevance==='not_ai'?'filtered':p.semanticState==='complete'&&p.relevance==='ai_relevant'?'processing':'retry';
 const review=w.db.prepare('SELECT r.* FROM acquisition_review_members m JOIN acquisition_cluster_reviews r ON r.cluster_id=m.cluster_id WHERE m.source_id=?').get(s.id);
 if(review?.status==='ignored')status='filtered';
 w.db.prepare("UPDATE intel_unified_sources SET status=?,attempts=attempts+1,updated_at=? WHERE source_id=?").run(status,now(),s.id);
 if(status==='processing')eligible.push(s);
 }
 const saved=[];let rejected=0,unchanged=0;
 if(eligible.length){
  for(const s of eligible)w.db.prepare('INSERT OR IGNORE INTO intel_run_sources(run_id,source_id) VALUES(?,?)').run(runId,s.id);
  ...
  try {
   const result=await generateDailyBriefs(w,env,intelligenceRun(w,runId),eligible,wiki,{...deps,unified:true});saved.push(...result.saved);rejected=result.rejected;unchanged=result.unchanged;
   for(const s of eligible){const brief=w.db.prepare("SELECT b.id,b.editorial_state FROM intel_briefs b,json_each(b.data_json,'$.evidence') e WHERE json_extract(e.value,'$.sourceId')=? ORDER BY b.updated_at DESC LIMIT 1").get(s.id);w.db.prepare("UPDATE intel_unified_sources SET status=?,brief_id=COALESCE(?,brief_id),last_error='',updated_at=? WHERE source_id=?").run(brief?brief.editorial_state==='ready'?'done':'retry':rejected?'retry':'filtered',brief?.id||null,now(),s.id);}
  } catch(error){if(error.cancelled||error.leaseLost)throw error;for(const s of eligible)w.db.prepare("UPDATE intel_unified_sources SET status='retry',last_error='解读或依据复核未完成',updated_at=? WHERE source_id=?").run(now(),s.id);}
 }
 w.db.prepare("UPDATE intel_unified_sources SET status='failed' WHERE status='retry' AND attempts>=2").run();
 const prior=stepState(w,runId,'unified'),summary={...unifiedSummary(w),newCount:(prior.newCount||0)+saved.filter(b=>b.version===1).length,updatedCount:(prior.updatedCount||0)+saved.filter(b=>b.version>1).length,unchanged:(prior.unchanged||0)+unchanged,rejected:(prior.rejected||0)+rejected};saveStep(w,runId,'unified','running',summary);
 if(summary.pending)return deferUnifiedRun(w,runId,`正在整理情报，剩余 ${summary.pending} 份资料`);
 const acquisition=stepState(w,runId,'acquire');const failures=acquisition.batchId?w.db.prepare("SELECT count(*) n FROM acquisition_runs WHERE batch_id=? AND status IN ('failed','blocked')").get(acquisition.batchId).n:0;
 saveStep(w,runId,'unified','done',{...summary,acquisitionFailures:failures});updateRun(w,runId,summary.failures||failures?'partial':'done',`情报更新完成：新增 ${summary.newCount} 条，更新 ${summary.updatedCount} 条${summary.failures||failures?'；部分来源或资料待复核':''}`);return intelligenceRun(w,runId);
}
