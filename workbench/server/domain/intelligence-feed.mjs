...
import { intelligenceAiConsent, saveIntelligenceAiConsent, intelligenceAutoUpdate, setIntelligenceAutoUpdate, RECOMMEND_WINDOW_MS, AUTO_UPDATE_INTERVAL_MS, creatorPlatforms, setCreatorPlatforms } from './intelligence-pool.mjs';
...
/**
 * 「今天值得做」：最近 72 小时、创作价值中或高、没被忽略的事件。
 * 分数 = 价值 × 时效紧迫度 × 信息差 + 热度/20。返回完整排序，前端按平台筛完再取前 5 个。
 */
export function rankOpportunities(briefs,{now=Date.now(),platform=null}={}){
 const at=typeof now==='number'?now:Date.parse(now);
 const value={high:3,medium:2},urgency={'24h':1.5,week:1,evergreen:0.8},gap={large:1.3,normal:1,saturated:0.6};
 const score=b=>{const c=b.event.creation;return value[c.value]*(urgency[c.window]||1)*(gap[c.zhGap]||1)+(b.event.heat||0)/20;};
 return briefs.filter(b=>b.event?.creation&&value[b.event.creation.value]&&b.editorialState==='ready'&&!b.dismissed&&at-Date.parse(b.event.latestAt||0)<=OPPORTUNITY_WINDOW_MS&&(!platform||b.event.creation.formats?.some(f=>f.platform===platform)))
...
export function intelligenceIntake(w,env={}){
 ...
 return {progress,platforms:creatorPlatforms(w),consent:intelligenceAiConsent(w),autoUpdate:intelligenceAutoUpdate(w),redditApproved:truthy(env.REDDIT_PAID_ACQUISITION_APPROVED)&&Boolean(env.BRIGHTDATA_API_KEY),reddit:reddit?{...reddit,lastSuccessAt:redditSuccess}:null,lastBatch};
}
/**
...
export function saveIntelligenceSettings(w,input,env={}){
 ...
 if(input.platforms!==undefined){if(!Array.isArray(input.platforms))throw bad('情报设置格式无效');setCreatorPlatforms(w,input.platforms);}
 let run=null;
