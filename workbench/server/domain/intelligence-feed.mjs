...
import { externalEvidence, assessBriefQuality, persistIntelligenceCluster, sourceFromRow, intelligenceQualitySummary } from './intelligence-quality.mjs';
...
function briefSourceMeta(w,evidence){return [...new Set(evidence.map(e=>e.sourceId))].map(id=>{const row=w.db.prepare('SELECT * FROM intel_sources WHERE id=?').get(id);return row?intelligenceSourceMeta(sourceFromRow(row)):intelligenceSourceMeta({id});});}
...
export function saveIntelligenceBriefs(w,runId,items,wikiItems=[],groups=null,scopeReviews=[]){if(!Array.isArray(items))throw bad('模型没有返回精选数组');const run=intelligenceRun(w,runId),byId=new Map(runSources(w,runId).map(s=>[s.id,s]));const saved=[],rejectionReasons=[],savedGroups=new Set();let rejected=0,unchanged=0,watchCount=0;
 ...
 if(old){const prev=JSON.parse(old.data_json);data.storyKey=prev.storyKey;const upgraded=old.editorial_state!=='ready'&&data.editorialState==='ready';if(upgraded&&!data.changeNote)data.changeNote='补齐主张、用途与证据范围复核';const hasNew=evidence.some(e=>!prev.evidence.some(p=>p.sourceId===e.sourceId&&p.quote===e.quote));if((!hasNew&&!upgraded)||!data.changeNote){unchanged++;continue;}data.evidence=[...new Map([...data.evidence,...prev.evidence].map(e=>[e.sourceId+'\n'+e.quote,e])).values()].slice(0,12);}
 const id=old?.id||createUlid(),version=(old?.version||0)+1,now=stamp(),edition=new Intl.DateTimeFormat('sv-SE',{timeZone:'Asia/Shanghai'}).format(new Date(run.createdAt));w.db.prepare('INSERT INTO intel_briefs(id,story_key,run_id,data_json,version,edition_date,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET run_id=excluded.run_id,data_json=excluded.data_json,version=excluded.version,edition_date=excluded.edition_date,updated_at=excluded.updated_at').run(id,old?.story_key||key,runId,JSON.stringify(data),version,edition,now,now);w.db.prepare('INSERT INTO intel_brief_versions(brief_id,version,run_id,data_json,created_at) VALUES(?,?,?,?,?)').run(id,version,runId,JSON.stringify(data),now);const clusterId=persistIntelligenceCluster(w,group||{key:data.storyKey,focus:data.title,relationship:'standalone'},(group?.sourceIds||evidence.map(e=>e.sourceId)).map(id=>byId.get(id)).filter(Boolean));w.db.prepare('UPDATE intel_briefs SET cluster_id=?,editorial_state=?,freshness_kind=? WHERE id=?').run(clusterId,data.editorialState,data.freshnessKind,id);saved.push(id);if(group)savedGroups.add(group.key);if(data.confidence==='watch')watchCount++;
...
export function feedbackIntelligenceBrief(w,id,input){
 object(input);const reasons=['irrelevant','too_generic','weak_evidence','seen','not_now'];
...
export function rankIntelligenceBriefs(briefs,edition){
 const candidates=briefs.filter(b=>b.editorialState==='ready'&&!b.dismissed&&b.editionDate===edition);
 const score=b=>(b.quality?.independentEvidenceCount||0)*2+(b.suggestedUses?.length||0)+(b.read?0:2)+(b.kind==='practice'?1:0);
 candidates.sort((a,b)=>score(b)-score(a)||b.updatedAt.localeCompare(a.updatedAt));
 const seen=new Set(),publishers=new Map(),result=[];
 for(const b of candidates){const group=b.clusterId||b.storyKey||b.id,publisher=b.sourceMeta.find(s=>s.originKind==='external')?.publisherKey||b.id;if(seen.has(group)||(publishers.get(publisher)||0)>=2)continue;seen.add(group);publishers.set(publisher,(publishers.get(publisher)||0)+1);result.push(b.id);if(result.length===8)break;}
 return result;
}
