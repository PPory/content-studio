...
export function intelligenceBrief(w,id){
 ...
 return {...brief,researchLinks:links,researchIds:links.map(link=>link.id)};
}
export function intelligenceFeed(w){
 const result=intelligenceFeedBase(w);
...
export function refreshIntelligenceFeed(w,{collect=true}={}){if(collect&&unifiedAvailable(w))w.db.prepare("UPDATE intel_unified_sources SET status='pending',attempts=0,last_error='',updated_at=? WHERE status='failed'").run(stamp());const prefs=feedPreferences(w);const existing=w.db.prepare("SELECT id FROM intel_profiles WHERE json_extract(config_json,'$.output')='briefs' ORDER BY created_at LIMIT 1").get();const profile=saveIntelligenceProfile(w,{...(existing?{id:existing.id}:{}),name:'个人情报精选',query:prefs.directions.join('；').slice(0,500),frequency:'manual',providers:['collected','local'],limit:8,output:'briefs',paidApproved:false,autoSocial:false});const run=enqueueIntelligence(w,profile.id);if(collect&&unifiedAvailable(w)&&!stepState(w,run.id,'acquire').batchId){const batch=startAcquisitionBatch(w,{confirmed:true,trigger:'intelligence'});saveStep(w,run.id,'acquire','running',{batchId:batch.id});}return run;}
...
export function rankIntelligenceBriefs(briefs,edition,limit=8){
 ...
 for(const b of candidates){const group=b.clusterId||b.storyKey||b.id,publisher=b.sourceMeta.find(s=>s.originKind==='external')?.publisherKey||b.id;if(seen.has(group)||(limit===8&&(publishers.get(publisher)||0)>=2))continue;seen.add(group);publishers.set(publisher,(publishers.get(publisher)||0)+1);result.push(b.id);if(result.length===limit)break;}
 return result;
}
