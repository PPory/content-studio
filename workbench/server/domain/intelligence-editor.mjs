...
export async function generateDailyBriefs(w,env,run,sources,wiki,deps={}) {
 ...
 const response=await (deps.completeJson||completeJson)(env,{
  ...
  ].join('\n'),user:JSON.stringify({step:"compose",groups,directions:preferences.directions,focus:run.config.query,period:run.createdAt,coverage:run.coverage,sources:excerpts,wiki,previous:recent,userDiscussionSignals:discussions,negativeFeedback:w.db.prepare("SELECT action,reason,count(*) count FROM intel_feedback WHERE value=1 AND reason IS NOT NULL GROUP BY action,reason").all()}),maxTokens:14000});
 deps.assertCurrent?.();
 ...
 const result=saveIntelligenceBriefs(w,run.id,response.data?.briefs,wiki,groups,scopeReviews);
 ...
 try {
  ...
  const fixed=saveIntelligenceBriefs(w,run.id,revised,wiki,groups,await reviewBriefScopes(w,env,run,revised,excerpts,deps));
