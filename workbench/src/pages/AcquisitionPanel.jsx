...
const STATUS = { OK:'OK', NO_NEW_ITEMS:'NO_NEW_ITEMS', AUTH_BLOCKED:'AUTH_BLOCKED', RATE_LIMITED:'RATE_LIMITED', TIMEOUT:'TIMEOUT', PARSE_FAILED:'PARSE_FAILED', SOURCE_UNAVAILABLE:'SOURCE_UNAVAILABLE', STALE_UPSTREAM:'STALE_UPSTREAM', public_feed: '公开订阅', approved: '访问已批准', needs_approval_and_credentials: '需要批准和凭据', retry: '等待重试',  never: '尚未运行', pending: '等待处理', queued: '等待处理', running: '采集中', success: '已完成', done: '已完成', partial: '部分完成', no_new: '本次无新增', no_change: '上游未变化', failed: '失败', cancelled: '已取消', blocked: '暂不可采集', paused: '已暂停', verified: '入口已验证', valid: '入口已验证', unverified: '入口待验证', needs_validation: '入口待验证', needs_credentials: '需要配置凭据', needs_approval: '需要访问批准', ready: '可采集', allowed: '访问已允许', not_required: '公开访问', denied: '未获访问许可', unavailable: '入口不可用', upstream_partial: '上游部分失败', upstream_failed: '上游失败', awaiting_upstream: '等待上游发布', rate_limited: '等待限流恢复', history_gap: '历史覆盖有缺口' };
...
function Stats({ value }) {
  const raw = object(value), stats = {...raw,found:raw.fetched ?? raw.found,new:raw.inserted ?? raw.new,duplicates:raw.duplicate ?? raw.duplicates}, names = { found: '发现', new: '新增', updated: '更新', duplicates: '重复', failed:'失败', summary: '仅摘要' };
