...
import { ErrorNote, Loading, ViewTabs } from '../components/ui.jsx';
...
export function AcquisitionPanel() {
  const [data,setData] = useState(null), [error,setError] = useState(null), [busy,setBusy] = useState(''), [choice,setChoice] = useState(null), [notice,setNotice] = useState(''), [search,setSearch] = useState(''), [filter,setFilter] = useState('all');
  ...
  const filtered = channels.filter(c => `${c.name} ${c.platform || ''} ${GROUPS[c.source_group] || ''}`.toLowerCase().includes(search.toLowerCase())).filter(c=>filter==='all' || (filter==='enabled' ? c.enabled : filter==='paused' ? !c.enabled : Boolean(c.last_error || c.lastError) || ['failed','blocked'].includes(c.healthStatus || c.health_status)));
  return <section className="acquisition-panel" aria-label="情报采集来源">
    ...
    {!data && !error ? <Loading rows={4}/> : data && <>
      <div className="intel-channel-toolbar"><ViewTabs items={[{key:"all",label:"全部信源"},{key:"enabled",label:"已启用"},{key:"paused",label:"未启用"},{key:"failed",label:"异常"}]} value={filter} onChange={setFilter} label="信源状态"/><label className="acquisition-search">查找来源<input type="search" value={search} onChange={e=>setSearch(e.target.value)} placeholder="来源、平台或分组"/></label></div>
      <p className="intel-v2-hint">{channels.length} 个采集入口 · {channels.filter(c=>c.enabled).length} 个实际启用</p>
      {!filtered.length && <p>没有匹配的来源。</p>}
      {Object.entries(GROUPS).map(([group,title]) => { const rows=filtered.filter(c=>(c.source_group || 'legacy')===group); return rows.length ? <section key={group} className="acquisition-group" aria-label={title}><h3>{title}<span>{rows.length}</span></h3><div className="intel-channel-columns" aria-hidden="true"><span>名称 / 访问状态</span><span>采集时间</span><span>运行状态</span><span>操作</span></div><div className="intel-v2-list intel-channel-rows">{rows.map(channel => <article key={channel.id} data-channel-id={channel.id}>
        ...
        <div className="intel-channel-state"><p className="acquisition-status">{channel.healthStatus || channel.health_status || "尚未检查"}</p><p className="intel-v2-hint">目标：{channel.desiredEnabled ?? channel.desired_enabled ? '启用' : '暂停'} · 实际：{channel.enabled ? '已启用' : '未启用'}{channel.platform ? ` · ${channel.platform}` : ''}</p></div>
