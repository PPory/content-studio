...
const STATUS = { public_feed: '公开订阅', approved: '访问已批准', needs_approval_and_credentials: '需要批准和凭据', retry: '等待重试',  never: '尚未运行', pending: '等待处理', queued: '等待处理', running: '采集中', success: '已完成', done: '已完成', partial: '部分完成', no_new: '本次无新增', no_change: '上游未变化', failed: '失败', cancelled: '已取消', blocked: '暂不可采集', paused: '已暂停', verified: '入口已验证', valid: '入口已验证', unverified: '入口待验证', needs_validation: '入口待验证', needs_credentials: '需要配置凭据', needs_approval: '需要访问批准', ready: '可采集', allowed: '访问已允许', not_required: '公开访问', denied: '未获访问许可', unavailable: '入口不可用', upstream_partial: '上游部分失败', upstream_failed: '上游失败', awaiting_upstream: '等待上游发布', rate_limited: '等待限流恢复', history_gap: '历史覆盖有缺口' };
...
function Stats({ value }) {
  const stats = object(value), names = { found: '发现', new: '新增', updated: '更新', duplicates: '重复', summary: '仅摘要' };
  ...
  return <section className="acquisition-panel" aria-label="情报采集来源">
    ...
    {!data && !error ? <Loading rows={4}/> : data && <>
      ...
      {Object.entries(GROUPS).map(([group,title]) => { const rows=filtered.filter(c=>(c.source_group || 'legacy')===group); return rows.length ? <section key={group} className="acquisition-group" aria-label={title}><h3>{title}<span>{rows.length}</span></h3><div className="intel-v2-list">{rows.map(channel => <article key={channel.id} data-channel-id={channel.id}>
        ...
        <p className="intel-v2-hint">目标：{channel.desiredEnabled ?? channel.desired_enabled ? '启用' : '暂停'} · 实际：{channel.enabled ? '已启用' : '未启用'}{channel.platform ? ` · ${channel.platform}` : ''}</p>
        <dl className="acquisition-times"><div><dt>最近检查</dt><dd>{time(channel.lastAttemptAt || channel.last_attempt_at)}</dd></div><div><dt>最近入库</dt><dd>{time(channel.last_ingest_at)}</dd></div><div><dt>上游变化</dt><dd>{time(channel.last_changed_at)}</dd></div><div><dt>下次运行</dt><dd>{channel.enabled ? time(channel.next_due_at) : '暂停中'}</dd></div></dl>
        <Stats value={channel.lastStats}/>{channel.source_group==='follow_builders' && <details className="acquisition-permissions"><summary>三个内容流与上游状态</summary>{Object.entries({'feed-x.json':'X 动态','feed-blogs.json':'博客全文','feed-podcasts.json':'播客转录','state-feed.json':'上游去重状态'}).map(([file,name])=>{const stream=(runs.find(r=>r.channel_id===channel.id&&r.coverage?.streams)?.coverage?.streams || {})[file];return <p className="intel-v2-hint" key={file}>{name} · {stream ? label(stream.status) : '尚未同步'}{stream?.generatedAt ? ` · 上游更新 ${time(stream.generatedAt)}` : ''}{stream?.errors?.length ? ` · ${stream.errors.map(e=>typeof e==='string'?e:e.message||JSON.stringify(e)).join('；')}` : ''}</p>;})}</details>}{(channel.last_error || channel.lastError) && <p className="intel-channel-error">{channel.last_error || channel.lastError}</p>}
        <div className="intel-v2-actions"><button className="btn btn-sm" disabled={Boolean(busy)} onClick={()=>act(channel,'validate')}>验证入口</button><button className="btn btn-sm" disabled={Boolean(busy)} onClick={()=>act(channel,'sync')}>同步一次</button><button className="text-action" disabled={Boolean(busy)} onClick={()=>act(channel,'backfill')}>补采历史</button>{channel.adapter==='community' && channel.platform!=='reddit' && !(channel.platform==='github' && /\/search\//.test(channel.endpoint || channel.url || '')) && <button className="text-action" disabled={Boolean(busy)} onClick={()=>{setError(null);setChoice({channel,action:'configure'});}}>修改入口</button>}<button className="text-action" disabled={Boolean(busy)} onClick={()=>act(channel,channel.desiredEnabled ?? channel.desired_enabled ? 'pause' : 'enable')}>{channel.desiredEnabled ?? channel.desired_enabled ? '暂停采集' : '申请启用'}</button></div>
