...
function NoteCard({ item, onUpdate, onRemove, onInsight, onTag, onNotice, onDirty }) {
  ...
  return <article className="note-card" aria-label={`记录 ${dateLabel(item.createdAt)}`}>
    <header className="note-card__header"><div><time dateTime={item.createdAt}>{dateLabel(item.createdAt)}</time>{item.pinned && <span className="note-card__pin">置顶</span>}</div><div className="note-card__actions"><button className="btn btn-sm note-insight-trigger" disabled={busy || editing} onClick={onInsight}>✧ AI 洞察</button><details className="note-menu" ref={menu}><summary aria-label="记录操作">···</summary><div className="note-menu__panel"><button disabled={busy} onClick={() => { closeMenu(); setEditing(true); }}>编辑</button><button disabled={busy} onClick={() => act(async () => { await navigator.clipboard.writeText(item.text); onNotice("已复制记录"); })}>复制</button><button disabled={busy} onClick={() => act(() => onUpdate(item, { pinned: !item.pinned }))}>{item.pinned ? "取消置顶" : "置顶"}</button><button disabled={busy} onClick={() => { closeMenu(); onInsight(); }}>AI 洞察</button><hr /><button className="note-menu__danger" disabled={busy} onClick={() => { closeMenu(); setConfirm(true); }}>删除</button></div></details></div></header>
    {editing ? <NoteComposer initial={item} onDirty={onDirty} autoFocus onCancel={() => setEditing(false)} onSave={async body => { await onUpdate(item, body); setEditing(false); }} /> : <><div className={`note-card__body${long && !expanded ? " note-card__body--collapsed" : ""}`} dangerouslySetInnerHTML={{ __html: renderMarkdown(item.text) }} />{long && <button className="note-expand" aria-expanded={expanded} onClick={() => setExpanded(value => !value)}>{expanded ? "收起" : "展开全文"}</button>}<div className="notes-tags">{(item.tags || []).map(value => <button className="notes-tag" key={value} onClick={() => onTag(value)}>#{value}</button>)}</div></>}
