...
export function IntelligenceTopics({state,onGo}) {
  ...
  return <div className="intelligence-v2 intel-topics-page"><IntelligenceNav current="intel-topics" onGo={onGo} />
    ...
    {!data ? <Loading rows={3}/> : !items.length ? <Empty><h2>这里还没有{filter==="all" ? "" : readinessLabel[filter]}选题</h2><p>在精选中选择一条或多条信息，预览选题后再保存。</p><button className="btn" onClick={()=>onGo("intel")}>阅读精选</button></Empty> : <div className="intel-v2-list">{items.map(item=><article key={`${item.kind}:${item.id}`}><header><span className="intel-v2-state" data-state={item.readiness}>{readinessLabel[item.readiness] || "待整理"}</span><h2>{item.workingTitle || item.title || item.coreClaim || item.core_claim || item.question}</h2></header><TopicContent item={item}/><div className="intel-v2-actions">{item.kind==="bridge" ? <button className="btn" onClick={()=>onGo("bridge",`opportunity:${item.id}`)}>打开知识选题</button> : item.researchId ? <button className="btn" onClick={()=>onGo("research",item.researchId)}>继续研究</button> : <button className="btn" disabled={busy} onClick={()=>setStarting(item)}>开始研究</button>}</div></article>)}</div>}
