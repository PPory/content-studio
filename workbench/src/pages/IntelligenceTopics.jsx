...
import { Empty, ErrorNote, Loading, ViewTabs } from "../components/ui.jsx";
...
export function IntelligenceTopics({state,onGo}) {
  ...
  return <div className="intel-workspace intelligence-v2 intel-topics-page"><IntelligenceNav current="intel-topics" onGo={onGo} />
    <IntelligenceHeader title="选题" aside={<button className="btn" onClick={()=>onGo("bridge")}>从已有知识探索</button>} />
    ...
    {!data ? <Loading rows={3}/> : !items.length ? <Empty><h2>这里还没有{filter==="all" ? "" : readinessLabel[filter]}选题</h2><p>在精选中选择一条或多条信息，预览选题后再保存。</p><button className="btn" onClick={()=>onGo("intel")}>阅读精选</button></Empty> : <div className="intel-v2-list intel-topic-table">
      <div className="intel-topic-columns" aria-hidden="true"><span>状态</span><span>选题 / 面向谁</span><span>切入角度</span><span>读者能获得什么</span><span>证据缺口</span><span>下一步</span></div>
      {items.map(item=><article key={`${item.kind}:${item.id}`}>
        <span className="intel-v2-state" data-state={item.readiness}>{readinessLabel[item.readiness] || "待整理"}</span>
        <div className="intel-topic-name"><h2>{item.workingTitle || item.title || item.coreClaim || item.core_claim || item.question}</h2><p>{item.audience || "受众待明确"}</p></div>
        <p className="intel-topic-cell" data-label="切入角度">{item.angle || item.coreClaim || "待展开"}</p>
        <p className="intel-topic-cell" data-label="读者价值">{item.deliverable || "待明确"}</p>
        <p className="intel-topic-cell" data-label="证据缺口">{textList(item.evidenceGaps).join("；") || "尚未记录证据缺口"}</p>
        <div className="intel-v2-actions">{item.kind==="bridge" ? <button className="btn btn-sm" onClick={()=>onGo("bridge",`opportunity:${item.id}`)}>打开知识选题</button> : item.researchId ? <button className="btn btn-sm" onClick={()=>onGo("research",item.researchId)}>继续研究</button> : <button className="btn btn-sm" disabled={busy} onClick={()=>setStarting(item)}>开始研究</button>}</div>
        <details className="intel-topic-evidence"><summary>研究依据与计划</summary><TopicContent item={item}/></details>
      </article>)}
    </div>}
