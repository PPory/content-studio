...
export function Today({ onGo, onChanged, onForceGo = onGo, registerNavigationGuard }) {
  ...
  return <section className="workspace-overview">
    <header className="overview-heading"><div><h1>从一个问题，开始今天</h1><p>选题、文章与阅读，都从这里继续。</p></div><NewContentButton label="直接写文章" onGo={onGo} onChanged={onChanged} /></header>
    <form className="overview-panel overview-capture" onSubmit={capture}><label htmlFor="home-thought">有什么想记下来的？</label><textarea id="home-thought" aria-label="记下灵感" value={text} onChange={e => setText(e.target.value)} rows={2} placeholder="一个想法、疑问，或者想继续研究的话题…" maxLength={100000} /><div className="overview-capture-footer"><span role="status">{status || "先记录，不用起标题或分类。"}{connections?.length ? <button type="button" className="btn btn-sm" onClick={() => setExpanded(!expanded)} aria-expanded={expanded}>发现 {connections.length} 条 Wiki 关联 ↗</button> : null}</span><button className="btn btn-primary" disabled={busy || !text.trim()}>留下这条想法</button></div>
    ...
    <section className="overview-panel overview-recent"><header><h2>最近打开</h2></header><Items items={(activity.opened || []).slice(0, 5)} onGo={openRecent} empty="打开选题、文章或资料后，会在这里留下入口。" /></section></div>
