...
const cardKind=item=>[item.event?eventKindLabel[item.event.kind]||"热点事件":item.deepRead?"深读 · 原文较早":freshnessLabel(item.freshnessKind),item.depth==="deep"?"已深读":"",item.readScope==="summary"?"仅基于摘要":"",item.changeNote?"有新进展":""].filter(Boolean).join(" · ");
...
export function IntelligenceUnified({view="intel",state,onGo}) {
  ...
  const reading=item=>item.event?<EventReading brief={item} onGo={onGo} onBlock={block} onRetryDeep={retryDeep} dense/>:null;
