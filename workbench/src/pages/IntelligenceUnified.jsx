...
import { EventReading, eventKindLabel, creationTags, formatText, platformLabel } from "../components/EventReading.jsx";
...
const cardKind=item=>item.event?.creation&&item.event.creation.value!=="low"?[...creationTags(item.event.creation),item.depth==="deep"?"已深读":""].filter(Boolean).join(" · "):[item.event?eventKindLabel[item.event.kind]||"热点事件":item.deepRead?"深读 · 原文较早":freshnessLabel(item.freshnessKind),item.depth==="deep"?"已深读":"",item.readScope==="summary"?"仅基于摘要":"",item.changeNote?"有新进展":""].filter(Boolean).join(" · ");
...
export function IntelligenceUnified({view="intel",state,onGo}) {
  ...
  const [tab,setTab]=useState(()=>stored().tab||"today"),[platform,setPlatform]=useState(()=>stored().platform||"all"),[scope,setScope]=useState(()=>stored().scope||"all");
  ...
  const [chat,setChat]=useState(false),[consent,setConsent]=useState(null),[platformDraft,setPlatformDraft]=useState([]);
  // 卡片 / 列表是显示偏好，和「选题」页同一颗开关、各页各存一份。
  const [layout,setLayout]=useLayoutMode("intel");
  const settingsRef=useRef(null),joinRef=useRef(null),correctionRef=useRef(null),consentRef=useRef(null),inflight=useRef(false),keys=useRef(pendingKeys());
  ...
  useEffect(()=>{try{sessionStorage.setItem(storageKey,JSON.stringify({tab,platform,scope,source,query,limit,selected,peekId}));}catch{}},[tab,platform,scope,source,query,limit,selected,peekId]);
  useEffect(()=>{if(settings){setDirections((data?.preferences?.directions||[]).join("\n"));setPlatformDraft(data?.intake?.platforms||["wechat","x","video"]);settingsRef.current?.showModal();}},[settings]);
  ...
  const reading=item=>item.event?<EventReading brief={item} onGo={onGo} onBlock={block} onRetryDeep={retryDeep} onAddFormat={f=>addWithFormat(item,f)} dense/>:null;
  ...
  // 「今天值得做」：服务端给完整排序，这里按平台筛完只取前 5 个。
  const platforms=data?.intake?.platforms||["wechat","x","video"];
  const fits=b=>platform==="all"||b.event?.creation?.formats?.some(f=>f.platform===platform);
  const opportunities=(data?.opportunityIds||[]).map(id=>all.find(b=>b.id===id)).filter(b=>b&&fits(b)).slice(0,5);
  const list=(tab==="today"?opportunities:tab==="saved"?all.filter(b=>b.saved):scope==="dismissed"?all.filter(b=>b.dismissed):scope==="history"?all:recommended)
  ...
  // 按一个具体做法加入选题：一个做法直接加，多个做法弹出选择（平台 · 形式 · 角度）。
  const formatsOf=item=>(item.event?.creation?.formats||[]).filter(f=>platform==="all"||f.platform===platform);
  const addWithFormat=(item,f)=>addTopic([item.id],null,f?{creation:{...f,window:item.event?.creation?.window}}:{});
  const topicControl=item=>{
    const topics=links(item),formats=formatsOf(item);
    if(!topics.length&&formats.length>1)return <AnchoredPopover className="topic-links" panelClassName="topic-links__menu topic-formats" label="选择做法加入选题" trigger="加入选题 ▾" panelRole="menu">{close=><>{formats.map(f=><button role="menuitem" key={`${f.platform}:${f.form}`} onClick={()=>{close();addWithFormat(item,f);}}><strong>{formatText(f)}</strong>{f.angle&&<span>{f.angle}</span>}</button>)}<button role="menuitem" onClick={()=>{close();addWithFormat(item,null);}}>不指定做法</button></>}</AnchoredPopover>;
    if(!topics.length)return <button className="btn btn-sm" disabled={!!busy} onClick={()=>addWithFormat(item,formats[0])}>加入选题</button>;
  ...
  const fitsLine=item=>{const c=item.event?.creation;if(!c||c.value==="low"||!c.formats?.length)return "";return `适合：${c.formats.map(formatText).join(" · ")}`;};
  const materials=item=>item.event?`${item.event.sourceCount} 个来源${item.event.discussionCount?` · ${item.event.discussionCount} 条讨论`:""} · ${sourceDate(item.primaryDate||item.event.latestAt)||"日期未知"}`:`${cardMeta(item)}${item.sourceCount?` · ${item.sourceCount} 份材料`:""}${item.editorialState!=="ready"?" · 待复核":""}`;
  // 卡片尺寸对齐「选题」页：两行标题、两行摘要、一行出处；「值得关注」留在侧栏详情里。
  const card=item=><article className={`brief-card ${item.read?"is-read":""} ${peekId===item.id?"is-active":""} ${selected.includes(item.id)?"is-picked":""}`} key={item.id} data-brief={item.id}><div className="brief-card__top"><span className="brief-card__kind">{cardKind(item)}</span>{pickBox(item)}</div><button className="brief-card__title" onClick={()=>setPeekId(item.id)}>{!item.read&&<span className="brief-card__dot" aria-label="未读"/>}{item.title}</button><p className="brief-card__summary">{tab==="today"&&formatsOf(item)[0]?.angle?`角度：${formatsOf(item)[0].angle}`:item.summary}</p>{fitsLine(item)&&<p className="brief-card__fits">{fitsLine(item)}</p>}<p className="brief-card__meta">{materials(item)}</p><footer>{controls(item)}</footer></article>;
  // 列表行走共用的 `.rows / .row`（Wiki、选题同一种），样式沿用旧精选页的 `.brief-rows`。
  const row=item=><div className={`row brief-row ${item.read?"is-read":""} ${peekId===item.id?"is-active":""} ${selected.includes(item.id)?"is-picked":""}`} key={item.id} data-brief={item.id}><div className="row-head">{pickBox(item)}<button type="button" className="row-title brief-row__open" onClick={()=>setPeekId(item.id)}>{!item.read&&<span className="brief-card__dot" aria-label="未读"/>}{item.title}</button><span className="brief-row__summary">{item.summary}</span><span className="row-meta"><span className="brief-card__meta">{cardKind(item)} · {materials(item)}</span><span className="intel-row__acts">{controls(item)}</span></span></div></div>;
  ...
  return <div className={`intel-workspace intel-feed intel-unified ${full?"intel-unified-full":""}`}>
    {!full?<>
      ...
      <div className="unified-toolbar">
        <ViewTabs items={[{key:"today",label:"今天值得做"},{key:"recommended",label:"全部热点"},{key:"saved",label:"已收藏"}]} value={tab} onChange={v=>change(()=>{setTab(v);setScope("all");})} label="情报范围"/>
      ...
      {tab==="today"&&data&&platforms.length>1&&<div className="intel-platforms" role="group" aria-label="按平台筛选">{["all",...platforms].map(p=><button key={p} type="button" className="chip" aria-pressed={platform===p} onClick={()=>change(()=>setPlatform(p))}>{p==="all"?"全部":platformLabel[p]}</button>)}</div>}
    ...
    {full?<>{detailError?<ErrorNote error={detailError} what="打开情报" onRetry={()=>setNonce(n=>n+1)}/>:!brief?<Loading rows={4}/>:<div className={`brief-detail-layout ${chat?"with-chat":""}`}><article className="brief-reading">{reading(brief)||<BriefReading brief={brief} onGo={onGo} onBlock={block}/>}{brief.reviewClusterId&&!brief.event&&<div className="unified-detail-feedback"><span>资料归到一起有误？</span><button className="text-action" onClick={()=>openCorrection(brief)}>纠正分组</button></div>}<IntelligenceAngles key={`${brief.id}:${brief.version}`} brief={brief} onChoose={angle=>addTopic([brief.id],angle)}/></article>{chat&&<aside className="brief-chat"><AssistantPane embedded scope="global" surface="page" scopeId={brief.scopeId||`intelligence:${brief.id}`} initialConversationId={brief.conversations?.[0]?.id||""} document={{title:brief.title,body:brief.body,intelligenceId:brief.id}} materials={[]} target={{kind:"none",editable:false}} draftStorageKey={`intelligence:${brief.id}`}/></aside>}</div>}</>:<>
      {!data&&!error?<Loading rows={5}/>:<div className={`intel-feed__body ${peekId?"has-peek":""}`}><div className="intel-feed__list">{!list.length?<Empty><h2>{tab==="today"&&!query?"今天没有特别值得做的":query||source!=="all"?"没有匹配的情报":tab==="saved"?"还没有收藏的情报":scope==="dismissed"?"没有已忽略的情报":"暂时没有符合条件的推荐"}</h2>{tab==="today"&&!query&&!needsConsent?<><p>{platform==="all"?"最近 72 小时里没有创作价值高、时效合适的事件。":`最近 72 小时里没有适合${platformLabel[platform]}的事件。`}热点都在「全部热点」里。</p><button className="btn" onClick={()=>change(()=>setTab("recommended"))}>看全部热点</button><button className="text-action" onClick={()=>onGo("intel-resources")}>查找原始资料</button></>:needsConsent&&tab!=="saved"&&!query&&source==="all"?<><p>最近 7 天的新资料需要你授权交给模型整理，才能变成情报卡。</p><button className="btn btn-primary" onClick={openConsent}>允许并开始整理</button><button className="text-action" onClick={()=>onGo("intel-resources")}>查找原始资料</button></>:<><p>推荐只收最近 7 天的内容，更早的在「筛选 → 历史」里；未完成筛选的内容不会冒充推荐。</p><button className="btn" onClick={()=>onGo("intel-resources")}>查找原始资料</button></>}</Empty>:(layout==="list"?<div className="rows brief-rows intel-rows">{visible.map(row)}</div>:<div className="brief-grid intel-card-grid">{visible.map(card)}</div>)}{list.length>limit&&<button className="btn unified-load-more" onClick={()=>setLimit(v=>v+8)}>查看更多（还有 {list.length-limit} 条）</button>}</div>{peekId&&<BriefPeek brief={brief} error={detailError?.message} loading={!brief&&!detailError} position={position+1} total={visible.length} busy={busy} onRetry={()=>setNonce(n=>n+1)} onClose={closePeek} onFull={openFull} onPrev={position>0?()=>step(-1):undefined} onNext={position>=0&&position<visible.length-1?()=>step(1):undefined} onFeedback={patch=>brief&&feedback(brief,patch)} onMerge={()=>brief&&(links(brief).length?onGo("research",links(brief)[0].id):addTopic([brief.id]))} mergeLabel={brief&&links(brief).length?"查看选题":"加入选题"} onCorrectGroup={brief?.reviewClusterId&&!brief.event?()=>openCorrection(brief):undefined} onGo={onGo} onBlock={block} reading={reading}/>}</div>}
    ...
    {settings&&<dialog className="brief-settings" ref={settingsRef} aria-label="关注方向" onCancel={()=>setSettings(false)}><h2>关注方向</h2>{error&&<ErrorNote error={error} what="保存关注方向"/>}<p>用于排序和解释价值，不会把无关内容放进 AI 推荐。</p><label>每行一个方向<textarea aria-label="关注方向内容" rows={6} value={directions} onChange={e=>setDirections(e.target.value)}/></label><fieldset className="intel-platform-settings"><legend>创作主战场（创作判断只在这些平台里给建议）</legend>{Object.entries(platformLabel).map(([key,label])=><label key={key}><input type="checkbox" checked={platformDraft.includes(key)} onChange={e=>setPlatformDraft(v=>e.target.checked?[...v,key]:v.filter(x=>x!==key))}/>{label}</label>)}</fieldset><footer><button className="btn btn-primary" disabled={!!busy} disabled={!!busy||!platformDraft.length} onClick={()=>act("settings",async()=>{await api.intelligencePreferences({directions:directions.split("\n").map(x=>x.trim()).filter(Boolean)});await api.intelligenceSaveSettings({platforms:platformDraft});setSettings(false);await load();})}>保存</button><button className="btn" onClick={()=>setSettings(false)}>取消</button></footer></dialog>}
