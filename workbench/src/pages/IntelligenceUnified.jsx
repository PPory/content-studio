...
import { IntelligenceDigest } from "../components/IntelligenceDigest.jsx";
...
const PAGE=30;
const firstTab=value=>["saved","digest"].includes(value)?value:"recommended";
...
export function IntelligenceUnified({view="intel",state,onGo}) {
  ...
  // 从一篇内容的「看原情报卡」进来：返回按钮回到那一篇（选题流程里记下的，见 StageRead.jsx），不是回情报列表。
  const returnTo=(()=>{if(!full)return null;try{const r=JSON.parse(sessionStorage.getItem("intel-return")||"null");return r?.view==="project"&&r.id?r:null;}catch{return null;}})();
  ...
  async function feedback(item,patch){const r=await act(item.id,()=>api.intelligenceFeedback(item.id,patch));if(!r)return;apply(r.brief);if(patch.dismissed){setNotice({text:"已忽略，可随时恢复",undo:()=>feedback(item,{dismissed:false})});if(peekId===item.id)setPeekId("");}else if(patch.saved===false)setNotice({text:"已移出收藏",undo:()=>feedback(item,{saved:true})});else setNotice({text:patch.dismissed===false?"已恢复推荐":patch.saved===undefined?"已记录":"已收藏"});}
  ...
  const openFull=()=>{try{sessionStorage.removeItem("intel-return");sessionStorage.setItem("intel-list-scroll",JSON.stringify({main:document.querySelector(".main")?.scrollTop||0,window:window.scrollY}));}catch{}onGo("intel-detail",peekId);};
  ...
  // 菜单项：一行动作名，需要时下面一行灰色小字说明它会发生什么。
  const menuItem=(label,hint,onClick)=><button role="menuitem" className="intel-menu-item" onClick={onClick}><span>{label}</span>{hint&&<small>{hint}</small>}</button>;
  // 收藏页里，「移出收藏」是常驻的文字按钮——一个实心书签看不出能点；「不感兴趣」在收藏页没有意义，不出现。
  const inSaved=tab==="saved";
  const controls=item=><>
    {inSaved?<button type="button" className="intel-act intel-act--text is-state" disabled={!!busy} onClick={()=>feedback(item,{saved:false})}>移出收藏</button>
    ...
    {topicControl(item)}
    <AnchoredPopover className="unified-card-more" panelClassName="unified-card-menu intel-menu" label="卡片操作" trigger={<IconDots aria-hidden="true" stroke={1.8}/>} panelRole="menu" align="end">{close=><>
      {inSaved?menuItem("移出收藏","可以撤销",()=>{close();feedback(item,{saved:false});})
        :item.dismissed?menuItem("恢复推荐","回到热点列表",()=>{close();feedback(item,{dismissed:false});})
        :menuItem("不感兴趣","从热点里移走，可以撤销",()=>{close();feedback(item,{dismissed:true});})}
      {menuItem("加入已有选题…","或补充你的想法",()=>{close();alternate([item.id]);})}
    </>}</AnchoredPopover>
  </>;
  ...
  // 页头菜单里的设置：关注方向（从「筛选」挪过来，它是设置不是筛选）、自动更新、AI 整理授权。
  const runUpdate=()=>act("update",async()=>{await api.intelligenceRefreshFeed();await load();setNotice({text:"正在更新情报，已有内容仍可阅读"});});
  // 速览里「在热点里」：切回热点页签并打开那张卡。
  const openCard=id=>{setTab("recommended");setScope("all");setSource("all");setWorth(false);setQuery("");setPeekId(id);};
  const toolItems=close=><><button role="menuitem" onClick={()=>{close();setSettings(true);}}>关注方向…</button>{intake?.consent?.publicSources&&<><button role="menuitemcheckbox" aria-checked={intake.autoUpdate!==false} onClick={()=>{close();act("auto",async()=>{await api.intelligenceSaveSettings({autoUpdate:intake.autoUpdate===false});await load();});}}>{intake.autoUpdate!==false?"✓ ":""}自动更新（每 6 小时）</button><button role="menuitem" onClick={()=>{close();openConsent();}}>AI 整理授权…</button></>}</>;
  return <div className={`intel-workspace intel-feed intel-unified ${full?"intel-unified-full":""}`}>
    {!full?<>
      <IntelligenceHeader title="情报" action={<><IntelligenceNav current="intel" onGo={onGo} extra={toolItems}/><button className="btn btn-primary" disabled={!!busy||updating} onClick={runUpdate}>{busy==="update"||updating?"正在更新…":"更新情报"}</button></>}/>
      <div className="unified-toolbar">
        <ViewTabs items={[{key:"recommended",label:"热点"},{key:"digest",label:"速览"},{key:"saved",label:"已收藏"}]} value={tab} onChange={v=>change(()=>{setTab(v);setScope("all");})} label="情报范围"/>
        <div className="unified-toolbar__actions">
          <SearchBox value={query} onChange={v=>change(()=>setQuery(v))} ariaLabel="搜索情报" placeholder="搜索情报"/>
          {tab!=="digest"&&<AnchoredPopover className="unified-filters" panelClassName="unified-filter-panel" label="筛选情报" trigger={scope!=="all"||source!=="all"||worth?"筛选 · 已启用":"筛选"}>
          ...
          {tab!=="digest"&&<LayoutToggle value={layout} onChange={setLayout}/>}
        </div>
      </div>
      {data&&<p className={`unified-update-status ${processingProblem||needsConsent||redditQuota?"has-problem":""}`}>{updating?<>{updateText}</>:needsConsent?<><span>{gap?`最近 7 天采到 ${gap} 条新资料，还没授权交给模型整理`:"新采集的资料需要授权后才能整理成情报"}</span><button className="text-action" onClick={openConsent}>允许并开始整理</button></>:redditQuota?<span>Reddit 额度不足，社区内容暂由 Hacker News 补位；额度恢复后自动切回</span>:processingProblem?<><span>{`${gap} 条近期资料还没授权整理`}</span><button className="text-action" onClick={openConsent}>查看授权</button></>:data.lastSuccessfulUpdate?<span>最近更新 {new Date(data.lastSuccessfulUpdate).toLocaleString("zh-CN",{month:"numeric",day:"numeric",hour:"2-digit",minute:"2-digit"})}</span>:<span>尚未更新</span>}</p>}
    </>:<header className="brief-detail-toolbar">{returnTo?<button className="btn" onClick={()=>{try{sessionStorage.removeItem("intel-return");}catch{}onGo(returnTo.view,returnTo.id);}} title={returnTo.title?`回到「${returnTo.title}」`:undefined}>← 回到这篇</button>:<button className="btn" onClick={()=>onGo("intel")}>← 返回情报</button>}<div>{brief&&controls(brief)}<button className="btn" onClick={()=>setChat(v=>!v)}>{chat?"收起讨论":"和 AI 聊聊"}</button></div></header>}
