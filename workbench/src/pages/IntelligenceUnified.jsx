...
const cardKind=item=>[item.deepRead?"深读 · 原文较早":freshnessLabel(item.freshnessKind),item.readScope==="summary"?"仅基于摘要":"",item.changeNote?"有新进展":""].filter(Boolean).join(" · ");
...
export function IntelligenceUnified({view="intel",state,onGo}) {
  ...
  const materials=item=>`${cardMeta(item)}${item.sourceCount?` · ${item.sourceCount} 份材料`:""}${item.editorialState!=="ready"?" · 待复核":""}`;
