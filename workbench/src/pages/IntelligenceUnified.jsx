...
import { EventReading, eventKindLabel } from "../components/EventReading.jsx";
...
/**
 * 卡片顶行只放会改变判断的东西：社区热议 / 实践这类类型，和「抢时效」。普通事件这一行是空的，不占位。
 * 旧流程的卡沿用原来的时效说明。
 */
const cardKind=item=>{
  if(!item.event)return [item.deepRead?"深读 · 原文较早":freshnessLabel(item.freshnessKind),item.readScope==="summary"?"仅基于摘要":"",item.changeNote?"有新进展":""].filter(Boolean).join(" · ");
  const c=item.event.creation;
  return [item.event.kind!=="event"?eventKindLabel[item.event.kind]:"",c&&c.value!=="low"&&c.window==="24h"?"抢时效":"",item.depth==="deep"?"已深读":""].filter(Boolean).join(" · ");
};
...
export function IntelligenceUnified({view="intel",state,onGo}) {
  ...
  const [tab,setTab]=useState(()=>stored().tab||"today"),[scope,setScope]=useState(()=>stored().scope||"all");
  ...
  // 「今天值得做」：服务端给完整排序，这里只取前 5 个。
  const opportunities=(data?.opportunityIds||[]).map(id=>all.find(b=>b.id===id)).filter(Boolean).slice(0,5);
  ...
  const closePeek=()=>{const old=peekId;setPeekId("");requestAnimationFrame(()=>document.querySelector(`[data-brief="${CSS.escape(old)}"] :is(.brief-card__title,.brief-row__open,.intel-today__title)`)?.focus({preventScroll:true}));};
  ...
  const topicControl=item=>{
    ...
    if(!topics.length)return <button className="btn btn-sm" disabled={!!busy} onClick={()=>addFromCreation(item)}>加入选题</button>;
  ...
  const card=item=>{const kind=cardKind(item);return <article className={`brief-card ${item.read?"is-read":""} ${peekId===item.id?"is-active":""} ${selected.includes(item.id)?"is-picked":""} ${kind?"":"has-no-top"}`} key={item.id} data-brief={item.id}>{kind?<div className="brief-card__top"><span className="brief-card__kind">{kind}</span>{pickBox(item)}</div>:pickBox(item)}<button className="brief-card__title" onClick={()=>setPeekId(item.id)}>{!item.read&&<span className="brief-card__dot" aria-label="未读"/>}{item.title}</button><p className="brief-card__summary">{item.summary}</p><p className="brief-card__meta">{materials(item)}</p><footer>{controls(item)}</footer></article>;};
  // 「今天值得做」是一份排好序的清单：序号、标题、切入角度、为什么值得做。第 1 名大一档，是这一屏最强的落点。
  const openRow=(event,item)=>{if(event.target.closest("button,a,input,label,[role=menu]"))return;setPeekId(item.id);};
  const todayRow=(item,index)=>{const c=item.event?.creation||{};const meta=[c.reason,item.event?`${item.event.sourceCount} 个来源`:"",item.event?.discussionCount?`${item.event.discussionCount} 条讨论`:""].filter(Boolean).join(" · ");return <li key={item.id} data-brief={item.id} className={`intel-today__item ${index===0?"is-first":""} ${item.read?"is-read":""} ${peekId===item.id?"is-active":""}`} onClick={e=>openRow(e,item)}><span className="intel-today__rank" aria-hidden="true">{index+1}</span><div className="intel-today__main"><button type="button" className="intel-today__title" onClick={()=>setPeekId(item.id)}>{!item.read&&<span className="brief-card__dot" aria-label="未读"/>}{item.title}</button>{c.angle&&<p className="intel-today__angle"><span>切入：</span>{c.angle}</p>}{meta&&<p className="intel-today__meta">{meta}</p>}</div><div className="intel-today__side">{c.window==="24h"&&<span className="intel-badge">抢时效</span>}<span className="intel-today__acts">{controls(item)}</span></div></li>;};
  // 列表行走共用的 `.rows / .row`（Wiki、选题同一种），样式沿用旧精选页的 `.brief-rows`。
  const row=item=><div className={`row brief-row ${item.read?"is-read":""} ${peekId===item.id?"is-active":""} ${selected.includes(item.id)?"is-picked":""}`} key={item.id} data-brief={item.id}><div className="row-head">{pickBox(item)}<button type="button" className="row-title brief-row__open" onClick={()=>setPeekId(item.id)}>{!item.read&&<span className="brief-card__dot" aria-label="未读"/>}{item.title}</button><span className="brief-row__summary">{item.summary}</span><span className="row-meta"><span className="brief-card__meta">{[cardKind(item),materials(item)].filter(Boolean).join(" · ")}</span><span className="intel-row__acts">{controls(item)}</span></span></div></div>;
    ...
    {!full?<>
      ...
      <div className="unified-toolbar">
        ...
        <div className="unified-toolbar__actions">
          ...
          {tab!=="today"&&<LayoutToggle value={layout} onChange={setLayout}/>}
        </div>
      </div>
