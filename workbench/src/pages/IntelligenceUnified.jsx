...
const cardKind=item=>item.deepRead?"深读 · 原文较早":freshnessLabel(item.freshnessKind);
...
export function IntelligenceUnified({view="intel",state,onGo}) {
  ...
  async function saveConsent(){const first=!intake?.consent?.publicSources;const r=await act("consent",()=>api.intelligenceSaveSettings(consent));if(!r)return;setConsent(null);await load();setNotice({text:r.run?"已开始整理最近 7 天的资料，已有内容仍可阅读":first?"已保存":"授权已更新"});}
