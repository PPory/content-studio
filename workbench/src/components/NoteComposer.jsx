...
export function NoteComposer({ initial = {}, onSave, onCancel, autoFocus = false, compact = false, onDirty }) {
  ...
  return <form className={`note-composer${compact ? " note-composer--compact" : ""}`} onSubmit={save}>
    <textarea data-autofocus ref={editor} autoFocus={autoFocus} aria-label="想留下什么" placeholder="现在的想法是……" rows={compact ? 6 : 4} value={text} disabled={busy} onChange={event => setText(event.target.value)} onKeyDown={event => { if ((event.ctrlKey || event.metaKey) && event.key === "Enter") save(event); }} />
