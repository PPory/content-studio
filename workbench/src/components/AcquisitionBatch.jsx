...
const counts = {fetched:'发现',inserted:'新增',updated:'更新',duplicate:'重复',failed:'失败'};
...
export function AcquisitionBatch({onGo,onLink}) {
  const [data,setData]=useState(null),[error,setError]=useState(null),[busy,setBusy]=useState(false),[group,setGroup]=useState(''),[stream,setStream]=useState(''),[offset,setOffset]=useState(0),[changesOnly,setChangesOnly]=useState(true);
  ...
  return <section className="acquisition-batch" aria-label="本次采集">
    <header className="intel-v2-toolbar"><div><h2>本次采集</h2><p className="intel-v2-hint">直接阅读本轮新增与更新的资料，核对正文和发现路径。</p></div><div className="intel-v2-actions"><button className="btn" disabled={busy} onClick={load}>刷新本次采集</button><button className="btn btn-primary" disabled={busy || active} onClick={sync}>{busy ? '正在安排…' : active ? '正在采集…' : '立即同步'}</button></div></header>
    ...
    {!data && !error ? <Loading rows={4}/> : !batch ? <Empty><h3>还没有整批采集记录</h3><p>点击「立即同步」后，将通过正式采集流程同步已启用的来源，结果会保存在这里。</p></Empty> : <>
      ...
      <dl className="acquisition-times"><div><dt>开始时间</dt><dd>{acquisitionTime(batch.startedAt)}</dd></div><div><dt>结束时间</dt><dd>{batch.finishedAt ? acquisitionTime(batch.finishedAt) : '尚未结束'}</dd></div><div><dt>耗时</dt><dd>{duration===null ? '—' : `${Math.floor(duration/60)} 分 ${duration%60} 秒`}</dd></div></dl>
      <dl className="acquisition-batch-totals">{Object.entries(counts).map(([key,label])=><div key={key}><dt>{label==='发现' ? '发现总数' : label}</dt><dd>{number(stats[key])}</dd></div>)}</dl>
      <div className="acquisition-batch-groups">{Object.entries(acquisitionGroups).map(([key,label])=>{const rows=streams.filter(row=>row.sourceGroup===key),members=channels.filter(row=>row.sourceGroup===key);return <section key={key} aria-label={`${label} 本轮结果`}><h3>{label}</h3>{rows.map(row=><div className="acquisition-stream-result" key={row.key}><strong>{row.label || row.key}</strong><span className="acquisition-status">{row.status}</span><CountRow stats={row.stats}/>{row.upstreamUpdatedAt && <span className="intel-v2-hint">上游更新 {acquisitionTime(row.upstreamUpdatedAt)}</span>}{row.error && <p className="intel-channel-error">{row.error}</p>}</div>)}{!rows.length && !members.length && <p className="intel-v2-hint">本轮没有此组的入口</p>}<details className="acquisition-channel-results"><summary>逐源结果与失败原因 · {members.length}</summary>{members.map(row=><div key={row.id}><strong>{row.name}</strong><span className="acquisition-status">{row.status}</span><CountRow stats={row.stats}/>{row.error && <p className="intel-channel-error">{row.error}</p>}<p className="intel-v2-hint">检查 {acquisitionTime(row.startedAt)} · 结束 {acquisitionTime(row.finishedAt)}</p></div>)}</details></section>;})}</div>
      ...
      {!items.length ? <p className="intel-v2-hint">{active ? '正在采集，取得资料后会显示在这里。' : '本轮所选范围没有新增或更新的资料。空批次不代表失败，请查看逐源状态。'}</p> : <div className="intel-v2-list acquisition-batch-items">{items.map(item=><AcquisitionResource key={item.id} item={{...item,type:'source',acquisitionBatch:true}} onGo={onGo} onLink={onLink}/>)}</div>}
