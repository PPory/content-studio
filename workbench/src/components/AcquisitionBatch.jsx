...
export function AcquisitionBatch({onGo,onLink}) {
  ...
  return <section className="acquisition-batch" aria-label="本次采集">
    ...
    {!data && !error ? <Loading rows={4}/> : !batch ? <Empty><h3>还没有整批采集记录</h3><p>点击「立即同步」后，将通过正式采集流程同步已启用的来源，结果会保存在这里。</p></Empty> : <>
      ...
      <p className="intel-v2-hint">本期深读允许近期上游新收录的较早博客与播客，与窗外数可能重叠，不应相加，也不计入过去 24 小时新资讯。</p><div className="acquisition-batch-groups">{Object.entries(acquisitionGroups).map(([key,label])=>{const rows=streams.filter(row=>row.sourceGroup===key),members=channels.filter(row=>row.sourceGroup===key);return <section key={key} aria-label={`${label} 本轮结果`}><h3>{label}</h3>{rows.map(row=><div className="acquisition-stream-result" key={row.key}><strong>{row.label || row.key}</strong><span className="acquisition-status">{row.status}</span><CountRow stats={row.stats}/>{row.existingMaterials!=null && <p className="intel-v2-hint">已有资料 {row.existingMaterials} 份 · 已取得正文 {row.existingFulltext} 份（不是本轮新增）</p>}{row.reason && <p className="intel-v2-hint">{row.reason}</p>}{row.upstreamUpdatedAt && <span className="intel-v2-hint">上游更新 {acquisitionTime(row.upstreamUpdatedAt)}</span>}{row.error && <p className="intel-channel-error">{row.error}</p>}</div>)}{!rows.length && !members.length && <p className="intel-v2-hint">本轮没有此组的入口</p>}<details className="acquisition-channel-results"><summary>逐源结果与失败原因 · {members.length}</summary>{members.map(row=><div key={row.id}><strong>{row.name}</strong><span className="acquisition-status">{row.status}</span><CountRow stats={row.stats}/>{row.reason && <p className="intel-v2-hint">{row.reason}</p>}{row.error && <p className="intel-channel-error">{row.error}</p>}{row.runs?.map(run=>run.coverage?.responseInfo && <p className="intel-v2-hint" key={run.id}>HTTP {run.coverage.responseInfo.httpStatus} · {run.coverage.responseInfo.contentType} · {run.coverage.responseInfo.finalUrl}</p>)}<p className="intel-v2-hint">检查 {acquisitionTime(row.startedAt)} · 结束 {acquisitionTime(row.finishedAt)}</p></div>)}</details></section>;})}</div>
