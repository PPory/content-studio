...
export function IntelligenceNav({current,onGo,extra}) {
  return <nav className="intel-section-nav" aria-label="情报工具">{current!=="intel"&&<a href="#/intel" onClick={event=>{if(event.button||event.ctrlKey||event.metaKey)return;event.preventDefault();onGo("intel");}}>← 返回情报</a>}<AnchoredPopover className="intel-nav-more" panelClassName="intel-nav-menu" label="情报工具" trigger={<IconDots aria-hidden="true" stroke={1.8}/>} panelRole="menu">{close=><>{destinations.map(([view,label])=><a role="menuitem" key={view} href={`#/${view}`} aria-current={view===current?"page":undefined} onClick={()=>close()}>{label}</a>)}{extra?.(close)}</>}</AnchoredPopover></nav>;
}
