...
export function DirectionBrowser({items,renderDetail,initialKey='',onReadingChange,onRemove}){
 ...
 return <section ref={root} className={`direction-browser ${selected?'is-reading':''}`} aria-label="方向浏览">
  ...
  {!selected&&<div className="direction-card-grid">{items.map(item=><article className="direction-overview-card" key={item.key} data-confirm={confirmKey===item.key?'':undefined}>
