// 第三步：补齐。只列选定角度真正缺的东西；每项说清为什么需要，操作只有三种：让 AI 找 / 我自己放 / 这条不写了。
...
export function StageGaps({ projectId, view, busy, onNext, onReload, onAsk, onAddMaterial }) {
  ...
  async function skip(g) {
    ...
    try { await markGap(projectId, g.label, { skip: true }); await onReload(); } catch (e) { setError(e); } finally { setSaving(""); }
  }
