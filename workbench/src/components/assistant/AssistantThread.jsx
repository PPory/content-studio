...
export function AssistantThread({
  ...
  return <div className="assistant-thread">
    ...
    {!messages.length && !busy && !loading ? <>{emptyMessage ? <p className="assistant-context-empty">{emptyMessage}</p> : <EmptyAssistant scope={scope} />}{starters}</> : null}
    {loading ? <Working label="正在打开对话" /> : null}
    {messages.map((item) => <div className="assistant-turn" key={item.id}><AssistantMessage item={item} attachments={attachments} currentVersion={currentVersion} capabilities={policy.capabilities} onRevise={(advice) => target.actions?.revise({ mode: "rewrite", label: "按建议改写", instruction: advice.slice(0, 2_000), selection: target.selection })} onInsert={(text) => target.actions?.insert(text, { ai: true, kind: "AI 助手候选", resultKind: "candidate", rerun: onRegenerate })} onReplaceBody={(text) => target.actions?.replaceBody(text, { kind: "AI 全文整理" })} onRegenerate={onRegenerate} onEdit={onEdit} latestAssistant={item.id === latestAssistantId} latestUser={item.id === latestUserId} working={busy && item.pending && !!item.text} activity={activity} showRuntime={showRuntime} />{onExcerpt && item.role === "assistant" && item.text && !item.pending ? <button type="button" className="btn btn-sm topic-excerpt" onClick={() => onExcerpt(item.text)}>摘进我的笔记</button> : null}{dedupeConsecutiveActionIds(item.actionIds, actions).map((id) => <ActionCard key={id} action={actions.find((action) => action.id === id)} onApply={onApplyAction} onReject={onRejectAction} onOpenRewrite={(rewrite) => target.actions?.replaceBody(rewrite.body, { kind: "AI 全文整理" })} originalLength={documentLength} inPiece={scope === "project"} />)}</div>)}
