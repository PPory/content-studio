 ...
 *
 * 平台仍是 Worker 的必填项，但不再每次弹出来问：它来自「我的创作」里的常用平台。
 * 发布前可以从项目页增加其他平台版本，日常新建因此只剩一次点击。
 */
export async function startWriting({
  ...
  const result = await creationApi.create({
     ...
     * ⚠️ **标题不拿正文的第一句顶上。**
     *
     * 从种子过来时正文第一段就是你记的那句话（或那条角度）。如果拿正文第一句
     * 自动推导标题，标题框里就会是一句四十字的判断，**你得先删掉它才能起标题，
     * 比空着还烦**。标题是要打磨的东西，空着（「未命名」）反而在提醒你
     * 「这儿还没定」；只有调用方明确给了标题时才使用。
     */
