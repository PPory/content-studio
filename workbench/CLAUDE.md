...
- ⚠️ **「新建」这类入口用 `MenuButton` 下拉，不直接开弹层。** 它省掉的是创作弹层里「起点选择」那**一整屏**（那一屏只干一件事：问三选一）。三条起点的真源是 `CreationDialog` 的 `MODES`，**别抄第二份**；**从下拉直接进的屏不画返回**（`preset === screen` 时 `back` 返回 null），否则会把人退到一屏他没见过的地方。
...
- ⚠️ **七条反应的文案从 `GET /wb/seeds` 的 `reactions` 来，前端一个字都不写死。**
