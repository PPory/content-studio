...
- ⚠️ **强调色只有一个：墨绿 `--brand`**，管三件事且**只管这三件**：主操作（`.btn-primary`）、进度（`.meter` / 进度环 / 流程线）、活跃状态。**`--accent` 那个黑不动**——黑管「位置和文字」（正文、边框、`--sel-bg`/`--sel-fg`），绿管「动作和进展」。**别把黑换成绿**：全换的话「你在这儿」和「你该点这儿」又是同一个视觉重量，只是把黑噪音换成绿噪音。
...
现役首页是 `Today.jsx`：先放一个最该推进的内容项目，再列后续项目，然后才是手写清单和后台流水线状态。项目优先级是**需处理 → 待发布 → 待诊断 → 写作中 → 待复盘 → 策划中**；生成中是流水线在做，不能装成用户待办。这只是界面排序，不是新的业务状态。
