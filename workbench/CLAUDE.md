...
- **内容项目页 = 顶上「需要你处理的」三张卡（`ProjectCard`）+ 下面一张全量表（`ProjectTable`），右上角切「看板」。** 展示件在 `pages/content/`。
  ...
  - ⚠️ **看板的合法转移只有五条**，真源是 `worker/src/lib/content-project.js` 的命令白名单 + `nextDraftWorkflow`。`ProjectBoard` 里那张 `TRANSITIONS` 是它的**镜像**，存在的唯一理由是**拖起来之前就知道哪些列能放**——服务端仍然会拒。改 Worker 白名单必须回头改这儿，漏改的表现是「界面允许拖、放下去 409」，看着像后端坏了。
...
- ⚠️ **阶段只在流程线上说。** 顶栏那颗 pill、流程线、右栏原来的 pill + 进度条是同一件事的三份；留流程线是因为**只有它同时答了「走到哪儿了」和「还剩几段」**，另外两份是它的子集。右栏从 `stageReason`（流程线答不了的「为什么在这一档」）开始说。
- ⚠️ **流程线走过的那几段是墨绿不是黑。** 黑在这套界面里是「你在这儿 / 点这儿」，一条线上七颗黑点等于同时在七处喊。节点里也不再写序号——线本身已经把顺序画出来了。
...
- ⚠️ **右栏是二选一**：平时是简报 + 素材（`.project-rail`），「待发布」那一档整个换成发布准备（`.project-publish`）。断言要写成二选一，钉死其中一种的话，库里第一条恰好走到待发布就红。
