...
支持两种模式：`serve` 读取手动刷新的脱敏快照；`serve-live` 每次查询通过独立、低优先级子进程只读访问当前 SQLite。二者都可通过官方 Secure MCP Tunnel 接入自己的 ChatGPT 工作区。

账号关联和 ChatGPT 对话调用须在实际目标账号单独验收；本机协议和隧道健康检查不代表对话验收。 模型是否支持该工具、账号是否具有开发者模式和隧道权限，必须在目标账号验证。不要把本地协议测试当成 ChatGPT 连接成功。
...
- 快照模式只接收快照目录；实时模式由主机显式传入生产根目录，查询子进程只读打开数据库。没有 SQL、路径、命令、URL 请求或写业务数据的工具。
