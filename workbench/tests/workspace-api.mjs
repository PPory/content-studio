...
try {
  ...
  const insightRun = await call(base, "/api/insights/run", { method: "POST", body: {} });
  assert.equal(insightRun.response.status, 409);
  const intelProfile = await call(base, "/api/workspace/intelligence/profiles", { method: "POST", body: { name:"本地测试",query:"本地素材",providers:["local"],frequency:"manual",limit:5 } });
  assert.equal(intelProfile.response.status, 200, JSON.stringify(intelProfile.value));
  const queued = await call(base, "/api/insights/run", { method:"POST", body:{profileId:intelProfile.value.profile.id} });
  assert.equal(queued.response.status,200,JSON.stringify(queued.value));
  const insights = await call(base, "/api/workspace/insights");
  check("旧洞察入口要求关注方向并进入调研队列，不再拼接素材报告", queued.value.run.status === "queued" && insights.value.reports.length === 0);
