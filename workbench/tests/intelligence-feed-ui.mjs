...
try{
 ...
 const provenance=await page.locator(".brief-peek .brief-provenance").first().innerText();assert(provenance.includes("X")&&provenance.includes("2026/09/07")&&provenance.includes("2026/09/08"),"原始发布日期和收集日期独立展示");assert((await page.locator(".brief-card.is-active").innerText()).includes(state.briefs[0].title),"正在读的那条在列表里高亮");await page.screenshot({path:path.join(shots,"intelligence-feed-peek.png"),fullPage:true});const foot=await page.locator(".brief-peek__foot").boundingBox();assert(foot.y+foot.height<=1001,`面板动作条掉出视口：${foot.y+foot.height} > 1000`);
 await page.keyboard.press("ArrowDown");await page.locator(".brief-peek").getByRole("heading",{name:state.briefs[1].title,exact:true}).waitFor();await page.keyboard.press("ArrowUp");await page.locator(".brief-peek").getByRole("heading",{name:state.briefs[0].title,exact:true}).waitFor();await page.keyboard.press("Escape");assert.equal(await page.locator(".brief-peek").count(),0,"Esc 关掉面板，列表位置不丢");
 // 卡片 / 列表双视图。列表里点标题照样开右侧面板，勾选和键盘流不变。
 await page.getByRole("button",{name:"列表视图",exact:true}).click();
 assert.equal(await page.locator(".brief-rows .row").count(),2,"切成列表之后条目一条不少");
 assert.equal(await page.locator(".brief-card").count(),0);
 await page.reload();await page.locator(".brief-rows .row").first().waitFor();
 assert.equal(await page.locator(".brief-card").count(),0,"刷新之后还是列表");
 await page.locator(".brief-row__open").first().click();
 await page.locator(".brief-peek").waitFor();
 assert.equal(await page.locator(".brief-row.is-active").count(),1,"列表里也能开 peek，并且当前那行高亮");
 await page.keyboard.press("Escape");
 await page.getByRole("button",{name:"卡片视图",exact:true}).click();
 assert.equal(await page.locator(".brief-card").count(),2,"切回卡片");
 await page.getByRole("button",{name:state.briefs[0].title,exact:true}).click();await page.locator(".brief-peek").getByRole("button",{name:"全屏打开",exact:true}).click();await page.waitForURL(/intel-detail/);
 await page.getByRole("heading",{name:"综合理解",exact:true}).waitFor();await page.getByText("资料 1 · 模型官方说明（测试）",{exact:true}).click();await page.getByText("原文发布时间未知",{exact:true}).waitFor();assert.equal(await page.locator(".brief-originals>details").count(),1);assert.equal(await page.locator(".brief-source-quotes blockquote").count(),3);assert.equal(await page.locator(".brief-source-record").count(),2);assert(await page.getByRole("button",{name:"屏蔽 example.com 网站",exact:true}).count());assert(!await page.getByText("细节默认折叠。",{exact:true}).isVisible());
 assert.equal(explores,0);await page.getByRole('button',{name:'探索表达角度',exact:true}).click();await page.getByRole('alert').waitFor();await page.getByRole('button',{name:'重试探索',exact:true}).click();await page.getByRole('heading',{name:'怎样判断模型改善了创作？',exact:true}).waitFor();assert.equal(explores,2);assert.equal(merged,null);await page.getByRole('button',{name:'探索表达角度',exact:true}).click();await page.getByRole('button',{name:'探索表达角度',exact:true}).click();assert.equal(explores,2);await page.getByRole('button',{name:'带入选题',exact:true}).click();await page.getByRole('button',{name:'确认保存选题',exact:true}).waitFor();await page.getByText('补充一次任务对照',{exact:true}).waitFor();await page.getByRole('button',{name:'放弃候选',exact:true}).click();assert.equal(merged,null);await page.goto(base+"/#/intel-detail/brief-one");await page.getByRole('button',{name:'探索表达角度',exact:true}).click();await page.getByRole('button',{name:'带入选题',exact:true}).waitFor();await page.screenshot({path:path.join(shots,'intelligence-angles-desktop.png'),fullPage:true});await page.setViewportSize({width:390,height:844});await page.getByRole('button',{name:'带入选题',exact:true}).scrollIntoViewIfNeeded();assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.screenshot({path:path.join(shots,'intelligence-angles-mobile.png'),fullPage:true});await page.setViewportSize({width:1440,height:1000});
 ...
 await page.goto(base+"/#/intel-settings");await page.getByRole("dialog",{name:"关注方向设置"}).waitFor();await page.getByLabel("关注方向",{exact:true}).fill("AI 模型进展\n内容创作方法");await page.getByRole("button",{name:"保存关注方向",exact:true}).click();await page.getByText("关注方向已保存",{exact:true}).waitFor();assert.equal(await page.locator("dialog[open]").count(),0,"存完就关，结果贴着保存动作");assert.equal(state.preferences.pilotOnly,true);assert(!await page.locator('select').count(),"关注设置没有定时配置");
 ...
 await page.setViewportSize({width:1440,height:1000});await page.goto(base+"/#/intel");await page.getByRole("tab",{name:"我的收藏",exact:true}).click();assert.equal(await page.locator(".brief-card").count(),1);await page.getByRole("tab",{name:"本期精选",exact:true}).click();await page.getByLabel(`选择：${state.briefs[0].title}`).check();await page.getByLabel(`选择：${state.briefs[1].title}`).check();await page.getByRole("button",{name:"一起展开成选题",exact:true}).click();await page.getByRole("button",{name:"确认保存选题",exact:true}).click();await page.getByRole("button",{name:"开始研究",exact:true}).click();await page.getByRole("button",{name:"确认开始研究",exact:true}).click();await page.waitForURL(/research-merged/);assert.deepEqual(merged.briefIds,["brief-one","brief-watch"]);
 await page.goto(base+"/#/intel-detail/brief-one");await page.getByRole('button',{name:'探索表达角度',exact:true}).click();await page.getByRole('button',{name:'带入选题',exact:true}).click();await page.getByRole('button',{name:'确认保存选题',exact:true}).click();await page.getByRole('button',{name:'开始研究',exact:true}).click();await page.getByRole('button',{name:'确认开始研究',exact:true}).click();await page.waitForURL(/research-merged/);assert(merged.evidenceGaps.includes('补充一次任务对照'));angleMode='empty';await page.goto(base+"/#/intel");await page.getByRole("heading",{name:"本期值得阅读",exact:true}).waitFor();await page.goto(base+"/#/intel-detail/brief-one");await page.getByRole('button',{name:'探索表达角度',exact:true}).click();await page.getByText('暂时没有自然的表达角度，可以先围绕这条情报继续讨论。',{exact:true}).waitFor();assert.equal(errors.length,0,errors.join("\n"));console.log("精选 UI：手动获取、可靠/观察、补看收藏、peek 键盘流与 Esc、详情直聊、反馈、方向弹层、周报、多条汇入与手机通过");
