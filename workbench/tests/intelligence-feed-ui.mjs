...
try{
 ...
 await page.goto(base+"/#/intel");await page.getByRole('tab',{name:'推荐',exact:true}).waitFor();assert.equal(refreshes,0,'打开首页不自动采集');
 ...
 await page.goto(base+'/#/intel-settings');await page.getByRole('dialog',{name:'关注方向',exact:true}).waitFor();await page.getByLabel('关注方向内容',{exact:true}).fill('AI 模型进展\n内容创作方法');await page.getByRole('button',{name:'保存关注方向',exact:true}).click();await page.locator('dialog[open]').waitFor({state:'detached'});assert.deepEqual(state.preferences.directions,['AI 模型进展','内容创作方法']);
