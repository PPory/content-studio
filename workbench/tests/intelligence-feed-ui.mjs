...
try{
 ...
 assert.equal(explores,0);await page.getByRole('button',{name:'探索表达角度',exact:true}).click();await page.getByRole('alert').waitFor();await page.getByRole('button',{name:'重试探索',exact:true}).click();await page.getByRole('heading',{name:'怎样判断模型改善了创作？',exact:true}).waitFor();assert.equal(explores,2);assert.equal(merged,null);await page.getByRole('button',{name:'探索表达角度',exact:true}).click();await page.getByRole('button',{name:'探索表达角度',exact:true}).click();assert.equal(explores,2);await page.getByRole('button',{name:'带入选题',exact:true}).click();assert.equal(await page.getByLabel('想研究的问题',{exact:true}).inputValue(),'怎样判断模型改善了创作？');await page.getByText('还需要验证：补充一次任务对照',{exact:true}).waitFor();await page.getByRole('button',{name:'取消',exact:true}).click();assert.equal(merged,null);await page.screenshot({path:path.join(shots,'intelligence-angles-desktop.png'),fullPage:true});await page.setViewportSize({width:390,height:844});await page.getByRole('button',{name:'带入选题',exact:true}).scrollIntoViewIfNeeded();assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.screenshot({path:path.join(shots,'intelligence-angles-mobile.png'),fullPage:true});await page.setViewportSize({width:1440,height:1000});
 ...
 await page.goto(base+"/#/intel-detail/brief-one");await page.getByRole('button',{name:'探索表达角度',exact:true}).click();await page.getByRole('button',{name:'带入选题',exact:true}).click();await page.getByRole('button',{name:'汇入并继续',exact:true}).click();await page.waitForURL(/research-merged/);assert.equal(merged.angle.gap,'补充一次任务对照');angleMode='empty';await page.goto(base+"/#/intel");await page.getByRole("heading",{name:"可靠信息",exact:true}).waitFor();await page.goto(base+"/#/intel-detail/brief-one");await page.getByRole('button',{name:'探索表达角度',exact:true}).click();await page.getByText('暂时没有自然的表达角度，可以先围绕这条情报继续讨论。',{exact:true}).waitFor();assert.equal(errors.length,0,errors.join("\n"));console.log("精选 UI：手动获取、可靠/观察、补看收藏、peek 键盘流与 Esc、详情直聊、反馈、方向弹层、周报、多条汇入与手机通过");
