...
try{
 ...
 await page.goto("http://127.0.0.1:5238/#/intel");assert.equal(await page.locator(".nav").count(),1,"侧栏DOM只有一份");await page.getByLabel("这次想了解什么？",{exact:true}).fill("GPT 模型进展和使用");
 ...
 await page.goto("http://127.0.0.1:5238/#/intel-settings");await page.getByText("调研记录",{exact:true}).waitFor();await page.locator(".intel-run summary").click();await page.getByPlaceholder("s_…").fill("s_existing");await page.getByRole("button",{name:"重试调研",exact:true}).click();assert.equal(retryBody.snapshotIds.x,"s_existing");
 await page.goto("http://127.0.0.1:5238/#/intel-inbox");assert(!await page.getByLabel("标题",{exact:true}).count(),"收集箱默认展示自动资料");await page.getByRole("button",{name:"我的灵感",exact:true}).click();await page.getByRole("button",{name:"记一条灵感",exact:true}).click();await page.getByLabel("标题",{exact:true}).fill("Harness 的疑问");await page.getByLabel("内容",{exact:true}).fill("为什么模型更强了，任务依旧可能失败？");failSave=true;await page.getByRole("button",{name:"保存灵感",exact:true}).click();await page.getByText("模拟收集失败",{exact:false}).waitFor();assert((await page.getByLabel("内容",{exact:true}).inputValue()).includes("任务"));failSave=false;await page.getByRole("button",{name:"保存灵感",exact:true}).click();await page.locator(".intel-source>summary").filter({hasText:"Harness 的疑问"}).waitFor();
 ...
 await page.goto("http://127.0.0.1:5238/#/intel");await page.getByRole("heading",{name:state.cards[0].question,exact:true}).waitFor();await page.locator(".intel-card>details:not(.intel-card-thinking)>summary").click();await page.getByText("已读取的内容",{exact:true}).waitFor();assert.equal(detailReads,0,"默认不读取来源全文");await page.getByText("已读取的内容",{exact:true}).click();await page.locator(".intel-source-body").filter({hasText:"全文结束标记"}).waitFor();assert.equal(detailReads,1,"展开按需读取全文");
 ...
 await page.goto("http://127.0.0.1:5238/#/intel-settings");await page.getByRole("button",{name:"编辑",exact:true}).click();await page.getByLabel("关注主题",{exact:true}).waitFor();assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.screenshot({path:path.join(shots,"intelligence-settings-mobile.png"),fullPage:true});
