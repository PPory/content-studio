...
try{
 ...
 await page.getByRole('button',{name:'查看灵感',exact:true}).click();await page.locator('.intel-v2-resource summary').filter({hasText:source.title}).click();await page.getByRole('link',{name:'查看原文 ↗',exact:true}).waitFor();await page.getByRole('button',{name:'带入选题',exact:true}).click();await page.getByLabel('想研究的问题',{exact:true}).fill('模型如何帮助实际工作？');
