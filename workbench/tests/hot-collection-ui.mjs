...
try{
 ...
 await page.getByRole('button',{name:'查看灵感',exact:true}).click();await page.getByRole('tab',{name:'全部资料',exact:true}).click();await page.locator('.intel-reader-index').getByRole('button',{name:new RegExp(source.title)}).click();await page.getByRole('link',{name:'查看原文 ↗',exact:true}).waitFor();await page.getByRole('button',{name:'带入选题',exact:true}).click();await page.getByLabel('想研究的问题',{exact:true}).fill('模型如何帮助实际工作？');
