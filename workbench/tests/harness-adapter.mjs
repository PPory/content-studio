...
assert.deepEqual(skills.items.map((item) => item.id).sort(), ["fact-check", "idea-dialogue", "interview-to-draft", "material-extraction", "material-gap", "publish-review", "topic-clustering", "xenho-quality-nine"], "项目 Harness Skill 没有从真实目录读取");
assert.equal(assistantExperts().items.length, 6, "六位 Xenho 专家没有从工作台预设读取");
assert.notEqual(documentVersion({ title: "A", body: "第一版" }), documentVersion({ title: "A", body: "第二版" }), "正文变化后版本号没有变化");
