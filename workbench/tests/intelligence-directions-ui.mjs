...
try{
 ...
 await page.goto('http://127.0.0.1:5243/#/intel');await page.locator('.subnav').getByRole('button',{name:'发现方向',exact:true}).click();await page.locator('.opportunity-home .view-tabs').waitFor();assert.equal(await page.locator('.view-head__name').innerText().then(t=>t.split('发现方向').length-1),1,'面包屑里页名只出现一次');assert.equal(await page.locator('.opportunity-home h2').filter({hasText:'发现方向'}).count(),0,'页面不再自我介绍');assert(await page.locator('.nav').innerText().then(t=>!t.includes('内容机会')));
