...
try{
 ...
 const connection=generated.connections[0];writeDiscoveryCache(w,{connections:[connection],read:context.read,scannedAt:new Date().toISOString(),fingerprint:context.fingerprint});
 ...
 await page.getByRole('heading',{name:'这个方向如何形成',exact:true}).waitFor();await page.getByText('查看原始记录',{exact:true}).click();await page.getByText(source.body,{exact:true}).waitFor();assert.deepEqual(counts(),before);
 ...
 await page.reload();await page.getByRole('navigation',{name:'方向视图'}).getByRole('button',{name:/已保存/}).click();await page.getByLabel('搜索已保存的机会').fill('不匹配的搜索');await page.getByText('没有找到匹配的机会，试试其他关键词。',{exact:true}).waitFor();await page.getByLabel('搜索已保存的机会').fill('');await page.locator('.direction-saved button').click();await page.getByRole('button',{name:'聊聊这个方向',exact:true}).click();await page.locator('.direction-chat textarea').waitFor();await page.getByRole('button',{name:'收起讨论',exact:true}).click();
 await page.setViewportSize({width:390,height:844});await page.getByRole('button',{name:'带入选题',exact:true}).scrollIntoViewIfNeeded();assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.screenshot({path:path.join(shots,'intelligence-directions-mobile.png'),fullPage:true});
