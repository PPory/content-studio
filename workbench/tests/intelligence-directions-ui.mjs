...
try{
 ...
 await page.goto('http://127.0.0.1:5243/#/intel');await page.locator('.subnav').getByRole('button',{name:'发现方向',exact:true}).click();await page.getByRole('heading',{name:'发现方向',exact:true}).waitFor();assert(await page.locator('.nav').innerText().then(t=>!t.includes('内容机会')));
