...
const shots=path.join(ROOT,'output/playwright/unified');await fs.mkdir(shots,{recursive:true});
...
const briefs=Array.from({length:11},(_,i)=>({id:`b${i}`,title:`模型更新的实际变化 ${i+1}`,summary:'对照任务显示改善，真实工作流仍待核对。',whyItMatters:'可用自己的重复任务做一次对照。',body:'原始说明表明模型在对照任务中改善。',uncertainties:['真实环境未验证'],evidence:[{sourceId:'raw1',quote:source.body}],sourceMeta:[source],sources:[source],sourceDocuments:[source],sourceCount:1,sourceGroups:i%2?['community']:['aihot'],editorialState:i===10?'needs_review':'ready',freshnessKind:'recent_event',read:false,saved:false,dismissed:false,version:1}));
const feed=()=>({ok:true,briefs,featuredIds:briefs.slice(0,8).map(b=>b.id),recommendationIds:briefs.slice(0,10).map(b=>b.id),activeRuns:[],preferences:{directions:['AI 实践']},processing:{newCount:10,updatedCount:0,pending:1,failures:1}});
...
try{
 ...
 browser=await chromium.launch();page=await browser.newPage({viewport:{width:1366,height:940}});const errors=[];page.on('pageerror',e=>errors.push(e.message));
 ...
 await page.route('**/api/workspace/intelligence**',async route=>{
  ...
  if(url.pathname.endsWith('/topics/intents')){calls++;lastIntent=body;assert.equal(body.confirmed,true);if(!operations.has(body.operationId)){operations.set(body.operationId,`research-${operations.size}`);researches.push({id:operations.get(body.operationId),title:briefs.find(b=>b.id===body.briefIds[0]).title,notes:body.notes||'',references:[],projects:[],conversations:[],updatedAt:'2026-09-21'});}return send(route,{research:{id:operations.get(body.operationId)}});}
 ...
 await first.locator('summary').click();await first.getByRole('button',{name:'忽略',exact:true}).click();await page.locator('[data-brief="b0"]').waitFor({state:'detached'});await page.getByRole('button',{name:'撤销',exact:true}).click();await page.locator('[data-brief="b0"]').waitFor();assert.equal(briefs[0].dismissed,false);
 await first.getByRole('button',{name:'加入选题',exact:true}).click();await page.getByRole('button',{name:'查看选题',exact:true}).waitFor();assert.match(page.url(),/#\/intel$/);assert.equal(operations.size,1);await first.getByRole('button',{name:'加入选题',exact:true}).click();await page.waitForTimeout(150);assert.equal(operations.size,1);assert.equal(previews,0);
 await page.locator('[data-brief="b1"] input').check();await page.locator('[data-brief="b2"] input').check();await page.getByRole('button',{name:'将所选情报加入一个选题',exact:true}).click();await page.waitForTimeout(150);assert.equal(lastIntent.briefIds.length,2);assert.equal(operations.size,2);
 await first.locator('summary').click();await first.getByRole('button',{name:'另选角度或加入已有选题',exact:true}).click();await page.getByLabel('保存到选题',{exact:true}).selectOption('existing');await page.getByLabel('我的选题想法',{exact:true}).fill('我想比较自己的工作流');await page.getByRole('button',{name:'保存选题',exact:true}).click();await page.waitForTimeout(150);assert.equal(lastIntent.researchId,'existing');assert.equal(lastIntent.notes,'我想比较自己的工作流');
 await first.locator('.brief-card__title').click();await page.getByRole('button',{name:'全屏打开',exact:true}).click();await page.getByRole('button',{name:'探索表达角度',exact:false}).click();await page.getByRole('button',{name:'带入选题',exact:true}).click();await page.waitForTimeout(150);assert.equal(lastIntent.angle.question,'这个改善能用于我的任务吗？');await page.screenshot({path:path.join(shots,'02-reading-detail.png'),fullPage:true});await page.getByRole('button',{name:'← 返回情报',exact:true}).click();await page.getByRole('button',{name:'关闭详情',exact:true}).click();assert.equal(await page.locator('.brief-card').count(),10);
 await page.reload();await page.locator('.brief-card').first().waitFor();assert.equal(await page.locator('.brief-card').count(),10);await page.screenshot({path:path.join(shots,'01-unified-feed.png'),fullPage:true});
 await page.goto('http://127.0.0.1:5276/#/intel-topics');await page.locator('.research-overview').waitFor();assert.match(page.url(),/#\/research/);assert.equal(previews,0);await page.screenshot({path:path.join(shots,'03-content-topics.png'),fullPage:true});
