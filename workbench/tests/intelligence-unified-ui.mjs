...
try{
 ...
 await page.getByRole('button',{name:'情报工具'}).click();const toolsText=await page.locator('.intel-nav-menu').innerText();assert.match(toolsText,/原始资料[\s\S]*处理详情[\s\S]*关注方向…/);assert.doesNotMatch(toolsText,/信源设置|每周回顾|AIhot 原始信息流/,'不起作用的入口从菜单去掉');await page.getByRole('menuitemcheckbox',{name:'✓ 自动更新（每 6 小时）'}).click();await page.waitForTimeout(100);assert.deepEqual(settingsBody,{autoUpdate:false});
