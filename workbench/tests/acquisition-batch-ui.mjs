...
try {
  ...
  const stats={fetched:8,inserted:1,updated:0,duplicate:7,failed:1};
  const batch={id:'fixture-ui-batch',startedAt:'2026-09-20T05:00:00Z',finishedAt:'2026-09-20T05:01:00Z',status:'partial',channelCount:2,stats,channels:[{id:follow.id,name:'Follow Builders',sourceGroup:'follow_builders',status:'OK',stats},{id:'reddit-blocked',name:'Reddit',sourceGroup:'community',status:'AUTH_BLOCKED',error:'缺少本轮 Reddit 访问授权',stats:{failed:1}}],streams:[{key:'follow_builders.podcast',label:'Podcast',sourceGroup:'follow_builders',status:'OK',stats},{key:'follow_builders.blog',label:'Blog',sourceGroup:'follow_builders',status:'NO_NEW_ITEMS',stats:{}}]};
  ...
  await page.goto(base+'/#/intel-resources');await page.getByRole('button',{name:'本次采集',exact:true}).click();
  ...
  await region.getByText('界面契约：完整 Podcast',{exact:true}).click();const card=region.locator('.intel-v2-resource');await card.getByText('TRANSCRIPT END',{exact:false}).first().waitFor();assert.equal(await card.locator(':scope>.intel-resource-body').textContent(),full);await card.getByText('采集详情',{exact:true}).click();await card.getByText('fixture-run',{exact:true}).waitFor();
  await fs.mkdir(shots,{recursive:true});await page.screenshot({path:path.join(shots,'acquisition-batch-desktop.png')});
  await region.getByLabel('本轮内容流',{exact:true}).selectOption('follow_builders.blog');await region.getByText('本轮所选范围没有新增或更新的资料。空批次不代表失败，请查看逐源状态。',{exact:true}).waitFor();
  await region.getByRole('button',{name:'立即同步',exact:true}).click();assert(synced);await region.getByLabel('本轮内容流',{exact:true}).selectOption('');
  await page.setViewportSize({width:390,height:844});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'no mobile overflow');await page.screenshot({path:path.join(shots,'acquisition-batch-mobile.png'),fullPage:true});
