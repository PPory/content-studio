...
try{
 ...
 await page.goto(base+"/#/intel-inbox");await page.getByLabel("标题",{exact:true}).fill("Harness 疑问");await page.getByLabel("内容",{exact:true}).fill("更强的模型为什么也会任务失败？这是我想进一步了解的问题。");await page.getByRole("button",{name:"保存到收集箱",exact:true}).click();await page.getByText("已收集 · 1",{exact:true}).waitFor();
 await page.goto(base+"/#/intel-settings/new");await page.getByLabel("方向名称",{exact:true}).fill("Harness 调研");await page.getByLabel("你想了解什么",{exact:true}).fill("Harness 上下文 工具 反馈");await page.getByLabel(/主题搜索/).uncheck();await page.getByRole("button",{name:"保存关注方向",exact:true}).click();await page.getByText("关注方向已保存",{exact:true}).waitFor();
 await page.getByRole("button",{name:"现在调研",exact:true}).click();const result=await until(()=>request("/api/workspace/intelligence"),r=>r.runs.some(x=>["done","partial","failed"].includes(x.status)),"真实任务完成");assert.equal(result.runs[0].status,"done",JSON.stringify(result.runs));assert.equal(result.cards.length,1);assert.equal(result.sources.length,2);
 await page.goto(base+"/#/intel");await page.getByRole("heading",{name:result.cards[0].question,exact:true}).waitFor();await page.locator(".intel-card>details>summary").click();await page.getByText("Harness 测试资料",{exact:true}).waitFor();
