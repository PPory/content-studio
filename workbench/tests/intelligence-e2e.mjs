...
try{
 ...
 await page.goto(base+"/#/intel-legacy/inbox");await page.locator(".intelligence").getByRole("button",{name:"我的灵感",exact:true}).click();await page.getByRole("button",{name:"记一条灵感",exact:true}).click();await page.getByLabel("标题",{exact:true}).fill("Harness 疑问");await page.getByLabel("内容",{exact:true}).fill("更强的模型为什么也会任务失败？这是我想进一步了解的问题。");await page.getByRole("button",{name:"保存灵感",exact:true}).click();await page.locator(".intel-source>summary").filter({hasText:"Harness 疑问"}).waitFor();
 await page.goto(base+"/#/intel-legacy");await page.getByLabel("这次想了解什么？",{exact:true}).fill("Harness 上下文与任务反馈");await page.getByRole("button",{name:"开始调研",exact:true}).click();const result=await until(()=>request("/api/workspace/intelligence"),r=>r.runs.some(x=>["done","partial","failed"].includes(x.status)),"真实任务完成");assert.equal(result.runs[0].status,"done",JSON.stringify(result.runs));assert.equal(result.cards.length,1);assert.equal(result.sources.length,2);
 await page.goto(base+"/#/intel-legacy");await page.getByRole("heading",{name:result.cards[0].question,exact:true}).waitFor();await page.locator(".intel-card>details:not(.intel-card-thinking)>summary").click();await page.getByText("Harness 测试资料",{exact:true}).waitFor();
