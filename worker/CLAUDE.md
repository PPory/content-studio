...
## 种子：这条链的新起点（`seeds` 表 + `lib/seeds.js`）

> **种子 = 你看到的东西 + 你对它的一句话**

整份设计见 [`../docs/工作流.md`](../docs/工作流.md)。这里只留改代码会踩的。

- **它解决的是「不知道自己能加什么」。** 「能加什么」的答案几乎总是你自己的经历和判断，
  而它只存在你脑子里——所以「选反应 + 补半句」这个动作的价值**不是分类，是把它问出来**。
  一旦补上「我不同意，因为___」，一篇短文的骨架当场就有了。
- ⚠️ **`take`（你那句话）必填，`reaction` 可以为空。** 有话说但不属于清单里那几种时，
  **硬塞一个分类比留空更糟**。清单是快车道，不是闸门。没有 take 就不成其为种子。
- ⚠️ **`reaction` 故意不加 CHECK 约束**，白名单只在 `lib/values.js` 的 `SEED_REACTIONS`。
  **那几条是文案，不是状态**——它不影响任何流转，只是提示语，而措辞和条数几乎肯定要调。
  钉进 CHECK 意味着改一个字就要重建表。写入只有我们自己的端点、它照白名单验，防护等价。
  **`status` 是真状态（影响流转），所以加 CHECK。**
  第一版上线当天就改过一次条数（见下条），当初不加 CHECK 那个决定当天就兑现了。
- ⚠️ **反应清单分三组，真源是 `SEED_REACTION_GROUPS`；扁平的 `SEED_REACTIONS` 从它算出来，
  不手写第二份。** 第一版七条**全都假设触发物是「一个观点」**，于是
  「某某发布了什么模型，我想写」一条都选不上——**你没法「同意」或「不同意」一件事实**，
  而热点里大量是发布和事件。补的那一组叫「对着一件事 / 一个发布」。
...
- ⚠️ **来源正文存在 `source_excerpt`，配一个 `source_fetched_at` 当「抓过没有」的开关。**
  ...
  只看正文空不空的话，一篇永远抓不到的文章会在每次打开项目页时被再抓一遍。
  `fetched_at > 0 且 excerpt 为空` = 抓过、确实抓不到。
- ⚠️ **触发物的标题和链接冗余存一份**（`source_title` / `source_url`），不能只存 id。
  热点**不在库里**——它是快照，会过期、会被覆盖。只存 id 的话，几天后这颗种子就说不清
  自己从哪来了。`source_id` 只在触发物真是库里的行时才有。
- ⚠️ **反应清单跟着 `GET /wb/seeds` 的响应一起回，工作台绝不抄第二份**
  （`sources.js` 那几处 `states` 抄了一份，对不上就是 400）。**零条种子时也要返回清单**，
  否则界面画不出选择器。响应里两份都给：`reactionGroups`（界面画这份）和
  `reactions`（拍平的，给校验和旧标签页）。
- ⚠️ **`GET /wb/projects/:id` 的载荷里带 `seed`**，按 `seeds.draft_id` 反查
  （`content-project.js` 的 `seedOfDrafts`，走 `idx_seeds_draft`）。项目页靠它显示
  「这一篇是从哪句话来的」。**必须从库里反查，不能靠前端交接**——交接条是一次性的，
  而你写到一半刷新是常事，刷完来源就没了。**列表接口不带它**（每个项目多一次查询，
  而列表页并不显示来源）。
- ⚠️ **`POST /wb/projects/:id/delete` 删的是选题那一行，而 `drafts.topic_id` 是
  `ON DELETE CASCADE`——它底下所有稿子跟着一起没**。所以这不是「删一个壳」，
  是删掉这一篇的全部内容；响应里回 `deleted`（删掉几篇）让界面照实说。
  ⚠️ **`vaultPaths` 必须在删之前一次性读出来，而且读的是稿子的、不是选题的**：
  选题没有 `vault_path`，有归档的是它底下那些稿；删完再查就什么都查不到，
  而 vault 里那几个文件还在——**连「对应哪条」都反查不回来**。
  动文件那一步只有工作台做得到（Worker 够不着你本机的 `.trash/`）。
- ⚠️ **`POST /wb/projects/:id/materials { add, remove }` 只接受「用户点过用这条」的。**
  `topic_materials` 的语义是**「这篇真的用了它」**——右栏的「已用 N 处」、归档时的
  证据链、复盘时的「有效故事」标记全建立在这个意思上。AI 挑出来的候选**不自动挂**，
  自动挂等于把「我用了」偷偷换成「系统猜它相关」，那三处从此都在说假话。
  **`remove` 不是可选的**：挂错一条却没有退路，比不给这个功能更糟。
- ⚠️ **不做 `task_key` 幂等。** 对同一条热点反应两次是**合法的**——那是两个不同角度，
  不该被去重挡住。防误双击是前端 disable 的事。
- **排序 `ORDER BY updated_at DESC, id DESC`**，理由和别的列表同一条（id 有 UUID/ULID
  两种格式，按 ASCII 比会把新建的整体压到迁移行后面）。
- ⚠️ **`source_kind` 是只写不读的溯源字段**，没有任何地方拿它做判断——界面按
  `source_title` / `source_url` 渲染。所以**加一类新来源不需要动它的 CHECK**
  （「找题」那一屏的洞察候选就用 `none` + 写清 `source_title`）。
  要动的话得重建表，而收益是零。

## 找题：两条只读端点（`lib/ideas.js` + `prompt/ideas-*.md`）

> **不存在「一键找选题」。** 一键产出的选题**不带你的判断**，而「不知道自己能加什么」
> 正是这条链要问出来的东西。这两条端点回的是**候选**，变成种子仍然要人补一句 take。

- `POST /wb/ideas/angles` 从一件事里拆**争点**（谁和谁在为什么吵）。
  ⚠️ **和反应清单不是一件事**：反应清单问「你什么反应」——那个问题在你还没找到抓手时
  答不上来；争点问「这件事的分歧在哪」，是**在你还没有反应的时候帮你找到可以有反应的地方**。
  只吃标题和摘要，**不去抓原文**（抓原文要 Readability，那是工作台那条路的事）。
- `POST /wb/ideas/materials` 把一段时间的素材聚成角度。
  ⚠️ **不写 topics 表。** `tasks/synthesize.js` 干的是同一类活但**写库**——
  写库会触发已有流转（选题进「撰写中」五分钟内自动成稿），而且 topics 是已作废的一层。
  ⚠️ **一个角度至少要连上两条素材**（`keepGroundedAngles`）：这条端点的全部价值是
  **跨条的连接**，只靠一条的话那条素材你自己翻的时候就看见了。
  ⚠️ **模型会编 material id**，照 `keepRealPicks` 的做法过滤——放行一个编造的 id，
  这个角度看起来「有依据」，而那份依据是假的。
- `POST /wb/ideas/cards` **一批角度 → 一批完整的选题卡**。三条来源共用这一个出口，
  而且 `ideas/angles` / `ideas/materials` **内部直接调它**——候选出来的那一刻
  就是一张完整的卡，不是让人点开再等一次。
  ⚠️ **出卡只能在这儿。** 卡上最值钱的一项是「手上有哪些素材能用」，
  它要求**同时看得到角度和素材库**——洞察那条 skill 跑在本机 vault 上够不着 D1，
  所以「让跑批时就写好卡」那条路**天然给不出这一项**。
  ⚠️ **一次调用出一批**，不是一条一次：8 条跑 8 次既慢又贵，而且模型看不到彼此、
  会给出几张几乎一样的卡。
  ⚠️ **卡片字段是白名单**（`lib/ideas.js` 的 `CARD_FIELDS`），
  **「传播潜力 / 紧急程度 / 热度」一个都不许加回来**——它们是模型对市场的猜测，
  使用者核实不了，而这条链上每一处都建立在「不让模型替你判断」上。
  摆一个「传播潜力：高」在卡上，一周之后他就会当它是真的。
  ⚠️ **`material_ids` 逐个在库里查得到才留**（`normalizeCard`，和 `keepRealPicks` 同一条）：
  编出来的 id 让这张卡**看起来有依据，而那份依据是假的**——比不给依据糟得多。
  ⚠️ **模型重排或漏给时按角度对齐，不按下标**：错位**不报错**，
  而你会照着一张不属于这个角度的卡去写。
- ⚠️ **三条都只读**，一行都不往库里写。**别顺手加一个「直接建选题」**——
  那正好是被否掉的那件事。
- 纯逻辑在 `lib/ideas.js` 而不是 `workbench.js`：后者 import 了 `prompt/*.md`
  （wrangler Text 模块），node 加载不了它，**放那儿等于这段校验没有测试**。
