...
- 稿件库 `drafts`：任务3 产出的初稿存这里。旧 `status` 只保留发布事实（待修改/已发布）；现役创作阶段看 `workflow_status`（写作中/待诊断/待发布/已发布/已弃用）。`topic_id` **可以为空**——`/推` 是绕过全流程的轻路径，产出的候选本来就没有选题。`topics.primary_draft_id` 是母版真源，`drafts.parent_draft_id` 记录变体父稿；阶段只能通过 `/wb/projects/:id/transition` 推进。发布复盘（链接、时间、五个数据指标、反馈状态）也在 drafts 上，由工作台的「记录发布」一次写入。部署这套代码前必须先应用 `migrations/0002_content_projects_v2.sql`。
...
- `GET /wb/projects?stage=&cursor=&pageSize=` 和 `GET /wb/projects/:id` 是内容项目只读聚合。它们不新建表，以 `topics` 为项目根、`draft:<id>` 表示孤立稿件，并批量带回真实关联的素材和来源。阶段只有策划中/生成中/写作中/待复盘/已完成/需处理；现有数据无法证明“待诊断”或“待发布”，不得猜。单稿是母版；多稿只在唯一匹配选题主平台时确定母版，其余返回“需确认母版”。列表的查询数是固定的，不得改回逐项查询。
