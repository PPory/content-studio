...
-- Agent 长任务的租约、心跳、重试和结构化结果；正文与发布状态不放在这里。
...
CREATE INDEX IF NOT EXISTS idx_agent_tasks_scope_updated ON agent_tasks(scope_id, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_agent_tasks_lease ON agent_tasks(status, lease_expires_at);

-- 外部正文提供者映射。正文仍由 provider 保存；这里只记录身份和同步摘要。
